...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_high_base_gate_mean": float(last.get("memory/high_base_gate_mean", 0.0)),
            "memory_effective_high_base_floor_gate_mean": float(last.get("memory/effective_high_base_floor_gate_mean", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_high_base_gate_mean",
        "memory_effective_high_base_floor_gate_mean",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate": summarize("sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate"),
}

report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# High-Base Floor-Gated Weak-Base Expert Keyed-Residual Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Best Train Return | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Gated Keyed Norm | Weak-Base Gate Mean | High-Base Gate Mean | Effective Floor Gate Mean | Keyed Scale Mean | Total Logits Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label, stats in variants.items():
    lines.append(
        "| {label} | {runs:.0f} | {eval_success_rate:.3f} | {eval_return:.3f} | {train_success_rate:.3f} | {best_train_return:.3f} | {throughput_fps:.1f} | {route_entropy:.3f} | {memory_route_bias_norm:.3f} | {memory_base_route_bias_logits_norm:.3f} | {memory_keyed_route_bias_logits_norm:.3f} | {memory_gated_keyed_route_bias_logits_norm:.3f} | {memory_weak_base_expert_gate_mean:.3f} | {memory_high_base_gate_mean:.3f} | {memory_effective_high_base_floor_gate_mean:.3f} | {memory_keyed_residual_scale_mean:.3f} | {memory_route_bias_logits_norm:.3f} |".format(
            label=label,
            **stats,
        )
    )
