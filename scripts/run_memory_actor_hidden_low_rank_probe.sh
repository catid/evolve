...
ROOT="outputs/experiments/memory_actor_hidden_low_rank_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_low_rank_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_low_rank_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_lowrank16.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_lowrank32.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_low_rank_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_low_rank_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_low_rank_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift_0.22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_lowrank16",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_low_rank_probe/memory_por_switchy_actor_hidden_partial_shift22_lowrank16"),
        "variant": "low_rank_16",
    },
    {
        "label": "por_actor_hidden_partial_shift22_lowrank32",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_low_rank_probe/memory_por_switchy_actor_hidden_partial_shift22_lowrank32"),
        "variant": "low_rank_32",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_low_rank": float(train_metrics.get("policy/option_hidden_low_rank", 0.0)),
            "policy/option_hidden_low_rank_dim": float(train_metrics.get("policy/option_hidden_low_rank_dim", 0.0)),
...
por_lowrank16 = row("por_actor_hidden_partial_shift22_lowrank16")
por_lowrank32 = row("por_actor_hidden_partial_shift22_lowrank32")
...
for item in (por_base, por_shift22, por_lowrank16, por_lowrank32):
    lines.append(
        f"| `{item['label']}` | `{item['greedy/success']:.4f}` | `{item['lower_t005/success']:.4f}` | `{item['gap_t0055/success']:.4f}` | `{item['shoulder_t008/success']:.4f}` | "
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_low_rank']:.1f}` | `{item['policy/option_hidden_low_rank_dim']:.0f}` | "
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_lowrank16` | `{por_lowrank16['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_lowrank16['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_lowrank16['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_lowrank16['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_lowrank32` | `{por_lowrank32['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_lowrank32['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_lowrank32['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_lowrank32['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)

interesting = (
    (por_lowrank16["greedy/success"] > 0.0 and (
        por_lowrank16["lower_t005/success"] > por_shift22["lower_t005/success"]
        or por_lowrank16["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    ))
    or (por_lowrank32["greedy/success"] > 0.0 and (
        por_lowrank32["lower_t005/success"] > por_shift22["lower_t005/success"]
        or por_lowrank32["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    ))
)
...
if interesting:
    lines.extend(
        [
            "## Outcome",
            "",
            "At least one low-rank variant preserved nonzero greedy conversion while improving part of the sampled band over `partial_shift22`, so this surface should get a longer confirmation pass before being accepted or rejected.",
        ]
    )
else:
    lines.extend(
