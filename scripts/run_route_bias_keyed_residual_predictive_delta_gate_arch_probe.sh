...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_delta_gate_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_gate_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_gate_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_gate_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_gate_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_delta_gate_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_delta_gate_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_delta_gate" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_positive_predictive_delta_logits_norm": float(last.get("memory/positive_predictive_delta_logits_norm", 0.0)),
            "memory_gated_positive_predictive_delta_logits_norm": float(last.get("memory/gated_positive_predictive_delta_logits_norm", 0.0)),
            "memory_predictive_delta_gate": float(last.get("memory/predictive_delta_gate", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_positive_predictive_delta_logits_norm",
        "memory_gated_positive_predictive_delta_logits_norm",
        "memory_predictive_delta_gate",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_delta_gate": summarize("sare_phase_memory_route_bias_keyed_residual_predictive_delta_gate"),
}

report_json.write_text(json.dumps({"runs": rows, "summary": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Delta-Gated Predictive Keyed-Residual Route-Bias Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Prior Keyed Norm | Predictive Keyed Norm | Final Keyed Norm | Positive Delta Norm | Gated Positive Delta Norm | Delta Gate | Total Logits Norm | Predictive Hidden Norm | Predictive Delta Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
