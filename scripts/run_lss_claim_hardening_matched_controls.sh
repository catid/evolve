...
SARE_ROOT="${PSMN_SARE_ROOT:-outputs/reproductions/lss_claim_hardening_baseline}"
OUTPUT_ROOT="${PSMN_OUTPUT_ROOT:-outputs/experiments/lss_claim_hardening/matched_controls}"
...
for seed in ${SEED_LIST}; do
  ...
  token_spec="${config_root}/kl_lss_token_dense.yaml"
  render_lss_spec \
    "$TEMPLATE" \
    "$seed_root" \
    "token_dense_ent1e3" \
    "seed_${seed}_kl_lss_token_dense" \
    "${out_seed_root}/kl_lss_token_dense" \
    "$token_spec"
  record_command "${out_seed_root}/kl_lss_token_dense/command.txt" python -m psmn_rl.analysis.learner_state_supervision run --spec "$token_spec" --device "$DEVICE"
  specs+=("$token_spec")

  if [[ "$RUN_SINGLE_EXPERT" == "1" ]]; then
    se_spec="${config_root}/kl_lss_single_expert.yaml"
    render_lss_spec \
      "$TEMPLATE" \
      "$seed_root" \
      "single_expert_ent1e3" \
      "seed_${seed}_kl_lss_single_expert" \
      "${out_seed_root}/kl_lss_single_expert" \
      "$se_spec"
    record_command "${out_seed_root}/kl_lss_single_expert/command.txt" python -m psmn_rl.analysis.learner_state_supervision run --spec "$se_spec" --device "$DEVICE"
    specs+=("$se_spec")
  fi
done
