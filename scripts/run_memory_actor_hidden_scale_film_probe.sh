...
ROOT="outputs/experiments/memory_actor_hidden_scale_film_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_scale_film_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_scale_film_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_scale_film.yaml"
...
manifest = {
    ...
    "family": "memory_actor_hidden_scale_film_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_film_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_film",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_film_probe/memory_por_switchy_actor_hidden_film"),
        "variant": "full_film",
    },
    {
        "label": "por_actor_hidden_scale_film",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_film_probe/memory_por_switchy_actor_hidden_scale_film"),
        "variant": "scale_only",
    },
    {
        "label": "por_actor_hidden_scale_film_mild",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_film_probe/memory_por_switchy_actor_hidden_scale_film_mild"),
...
por_film = row("por_actor_hidden_film")
por_scale = row("por_actor_hidden_scale_film")
...
for item in (por_base, por_film, por_scale, por_scale_mild):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | `{item['policy/option_hidden_film_scale_only']:.0f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_film` | `{por_film['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_film['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_film['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_film['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_scale_film` | `{por_scale['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_scale['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_scale['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_scale['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        ...
        "| --- | ---: | ---: | ---: |",
        f"| `por_actor_hidden_scale_film` | `{por_scale['lower_t005/success'] - por_film['lower_t005/success']:+.4f}` | `{por_scale['gap_t0055/success'] - por_film['gap_t0055/success']:+.4f}` | `{por_scale['shoulder_t008/success'] - por_film['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_scale_film_mild` | `{por_scale_mild['lower_t005/success'] - por_film['lower_t005/success']:+.4f}` | `{por_scale_mild['gap_t0055/success'] - por_film['gap_t0055/success']:+.4f}` | `{por_scale_mild['shoulder_t008/success'] - por_film['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
