...
python -m psmn_rl.analysis.lss_hard_family_program successor-pack \
  --campaign-config "$CAMPAIGN_CONFIG" \
  --stage3-json "$STAGE3_JSON" \
  --stage4-json "$STAGE4_JSON" \
  --stage5-json "$STAGE5_JSON" \
  --stage6-json "$STAGE6_JSON" \
  --stage7-json "$STAGE7_JSON" \
...
if [[ -f "$GATE_JSON" ]]; then
  python -m psmn_rl.analysis.lss_hard_family_program decision-memo \
