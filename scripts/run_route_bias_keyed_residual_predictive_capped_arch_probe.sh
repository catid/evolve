...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_capped_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_capped_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_capped_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_capped_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_capped_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_capped_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_capped_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_capped" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_prior_keyed_route_bias_logits_norm": float(last.get("memory/prior_keyed_route_bias_logits_norm", 0.0)),
            "memory_predictive_keyed_route_bias_logits_norm": float(last.get("memory/predictive_keyed_route_bias_logits_norm", 0.0)),
            "memory_keyed_route_bias_logits_norm": float(last.get("memory/keyed_route_bias_logits_norm", 0.0)),
            "memory_capped_predictive_delta_logits_norm": float(last.get("memory/capped_predictive_delta_logits_norm", 0.0)),
            "memory_route_bias_logits_norm": float(last.get("memory/route_bias_logits_norm", 0.0)),
            "memory_predictive_hidden_norm": float(last.get("memory/predictive_hidden_norm", 0.0)),
            "memory_keyed_predictive_delta_norm": float(last.get("memory/keyed_predictive_delta_norm", 0.0)),
            "memory_predictive_delta_scale": float(last.get("memory/predictive_delta_scale", 0.0)),
        }
    )
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_prior_keyed_route_bias_logits_norm",
        "memory_predictive_keyed_route_bias_logits_norm",
        "memory_keyed_route_bias_logits_norm",
        "memory_capped_predictive_delta_logits_norm",
        "memory_route_bias_logits_norm",
        "memory_predictive_hidden_norm",
        "memory_keyed_predictive_delta_norm",
        "memory_predictive_delta_scale",
    ]
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_capped": summarize("sare_phase_memory_route_bias_keyed_residual_predictive_capped"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Capped Predictive Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_predictive_capped`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Prior Keyed Norm | Predictive Keyed Norm | Final Keyed Norm | Capped Delta Norm | Total Logits Norm | Predictive Delta Norm | Predictive Delta Scale |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
    ...
    lines.append(
        "| "
        + f"`{label}` | `{int(summary.get('runs', 0))}` | "
        + f"`{summary.get('eval_success_rate', 0.0):.3f}` | "
        + f"`{summary.get('eval_return', 0.0):.3f}` | "
        + f"`{summary.get('train_success_rate', 0.0):.3f}` | "
        + f"`{summary.get('throughput_fps', 0.1):.1f}` | "
        + f"`{summary.get('route_entropy', 0.0):.3f}` | "
        + f"`{summary.get('memory_route_bias_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_base_route_bias_logits_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_prior_keyed_route_bias_logits_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_predictive_keyed_route_bias_logits_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_keyed_route_bias_logits_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_capped_predictive_delta_logits_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_route_bias_logits_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_keyed_predictive_delta_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_predictive_delta_scale', 0.0):.3f}` |"
    )
...
lines.extend(
    [
        ...
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Prior Keyed Norm | Predictive Keyed Norm | Capped Delta Norm | Predictive Delta Scale | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in sorted(rows, key=lambda item: (item["variant"], item["seed"])):
    lines.append(
        "| "
        + f"`{row['variant']}` | `{row['seed']}` | "
        + f"`{row['eval_success_rate']:.3f}` | "
        + f"`{row['eval_return']:.3f}` | "
        + f"`{row['train_success_rate']:.3f}` | "
        + f"`{row['throughput_fps']:.1f}` | "
        + f"`{row['memory_prior_keyed_route_bias_logits_norm']:.3f}` | "
        + f"`{row['memory_predictive_keyed_route_bias_logits_norm']:.3f}` | "
        + f"`{row['memory_capped_predictive_delta_logits_norm']:.3f}` | "
        + f"`{row['memory_predictive_delta_scale']:.3f}` | "
        + f"`{row['run_dir']}` |"
    )
