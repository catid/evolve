...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_base_delta_weak_base_gate_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_base_delta_weak_base_gate" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_predictive_base_weak_gate": float(last.get("memory/predictive_base_weak_gate", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_predictive_base_weak_gate",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_base_delta_weak_base_gate": summarize(
...
lines = [
    "# Predictive Base Delta Weak-Base-Gated Keyed-Residual Route-Bias Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Predictive Base Logits Norm | Final Keyed Norm | Predictive Base Delta Norm | Mixed Predictive Base Delta Norm | Predictive Base Weak Gate | Total Logits Norm | Predictive Hidden Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]

for label, stats in variants.items():
    lines.append(
        "| {label} | {runs:.0f} | {eval_success_rate:.3f} | {eval_return:.3f} | {train_success_rate:.3f} | {throughput_fps:.1f} | {route_entropy:.3f} | {memory_route_bias_norm:.3f} | {memory_base_route_bias_logits_norm:.3f} | {memory_predictive_base_route_bias_logits_norm:.3f} | {memory_keyed_route_bias_logits_norm:.3f} | {memory_predictive_base_delta_logits_norm:.3f} | {memory_mixed_predictive_base_delta_logits_norm:.3f} | {memory_predictive_base_weak_gate:.3f} | {memory_route_bias_logits_norm:.3f} | {memory_predictive_hidden_norm:.3f} |".format(
            label=label,
            **stats,
        )
    )
