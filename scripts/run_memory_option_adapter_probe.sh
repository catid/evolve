...
ROOT="outputs/experiments/memory_option_adapter_probe"
REPORT_MD="outputs/reports/memory_option_adapter_probe.md"
REPORT_JSON="outputs/reports/memory_option_adapter_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_option_adapter.yaml"
  "configs/experiments/minigrid_memory_por_switchy_option_adapter_small.yaml"
...
manifest = {
    ...
    "family": "memory_option_adapter_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_adapter_probe/memory_por_switchy_rerun"),
        "adapter": "none",
    },
    {
        "label": "por_adapter",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_adapter_probe/memory_por_switchy_option_adapter"),
        "adapter": "scale_0.5",
    },
    {
        "label": "por_adapter_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_adapter_probe/memory_por_switchy_option_adapter_small"),
        "adapter": "scale_0.2",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_option_adapter_probe/memory_token_gru_long_rerun"),
        "adapter": "none",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_action_adapter_stability": float(train_metrics.get("policy/option_action_adapter_stability", 0.0)),
            "policy/option_action_adapter_bias_norm": float(train_metrics.get("policy/option_action_adapter_bias_norm", 0.0)),
        }
...
por_adapter = row("por_adapter")
por_adapter_small = row("por_adapter_small")
gru_base = row("gru_base")

lines = [
    "# Memory Option-Adapter Probe",
    "",
    "- hypothesis: a POR-specific option-aware action adapter can preserve the sampled Memory band while improving deterministic decode",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Adapter Stability | Adapter Bias Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_adapter, por_adapter_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_action_adapter_stability']:.4f}` | `{item['policy/option_action_adapter_bias_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_adapter` | `{por_adapter['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_adapter['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_adapter['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_adapter['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_adapter_small` | `{por_adapter_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_adapter_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_adapter_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_adapter_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a reused-checkpoint sweep.",
        "- The adapter only counts as interesting if it improves the sampled band or greedy decode relative to the matched POR base rerun.",
    ]
)
