...
ROOT="outputs/experiments/memory_actor_hidden_shift_gate_power_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_shift_gate_power_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_shift_gate_power_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftgate125.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftgate150.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_shift_gate_power_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_power_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_power_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate125",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_power_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate125"),
        "variant": "shift_gate_power_1.25",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate150",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_power_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate150"),
        "variant": "shift_gate_power_1.50",
    },
]
...
por_gate125 = row("por_actor_hidden_partial_shift22_shiftgate125")
por_gate150 = row("por_actor_hidden_partial_shift22_shiftgate150")

lines = [
    "# Memory Actor-Hidden Shift-Gate Power Probe",
    "",
    "- hypothesis: the actor-hidden FiLM shift branch may be too active on middling duration states; sharpening only the shift-branch duration gate could preserve or recover greedy conversion by making the additive intervention more selective without touching the multiplicative scale branch",
...
for item in (por_base, por_shift22, por_gate125, por_gate150):
    lines.append(
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_shiftgate125` | `{por_gate125['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate125['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate125['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate125['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_shiftgate150` | `{por_gate150['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate150['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate150['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate150['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_gate125, por_gate150)
)

if interesting:
    lines.append("- conclusion: at least one shift-gate power variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: sharpening the actor-hidden shift-branch duration gate does not improve `partial_shift22`; this local surface does not benefit from a more selective additive gate either")
