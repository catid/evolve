...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_entropy_gate_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_entropy_gate_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_entropy_gate_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_entropy_gate_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_entropy_gate_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_entropy_gate_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_entropy_gate_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_entropy_gate" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_base_route_bias_entropy": float(last.get("memory/base_route_bias_entropy", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_base_route_bias_entropy",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_entropy_gate": summarize("sare_phase_memory_route_bias_keyed_residual_entropy_gate"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Entropy-Gated Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_entropy_gate`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Gated Keyed Norm | Total Logits Norm | Base Entropy | Gate Mean |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
    ...
    if not stats:
        continue
    lines.append(
        f"| `{label}` | `{stats['runs']:.0f}` | `{stats['eval_success_rate']:.3f}` | `{stats['eval_return']:.3f}` | `{stats['train_success_rate']:.3f}` | `{stats['throughput_fps']:.1f}` | `{stats['route_entropy']:.3f}` | `{stats['memory_route_bias_norm']:.3f}` | `{stats['memory_base_route_bias_logits_norm']:.3f}` | `{stats['memory_keyed_route_bias_logits_norm']:.3f}` | `{stats['memory_gated_keyed_route_bias_logits_norm']:.3f}` | `{stats['memory_route_bias_logits_norm']:.3f}` | `{stats['memory_base_route_bias_entropy']:.3f}` | `{stats['memory_keyed_residual_gate_mean']:.3f}` |"
    )
lines.extend(
    [
        "",
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Route Bias Norm | Keyed Logits Norm | Gated Keyed Norm | Base Entropy | Gate Mean | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in rows:
    lines.append(
        f"| `{row['variant']}` | `{row['seed']}` | `{row['eval_success_rate']:.3f}` | `{row['eval_return']:.3f}` | `{row['train_success_rate']:.3f}` | `{row['throughput_fps']:.1f}` | `{row['memory_route_bias_norm']:.3f}` | `{row['memory_keyed_route_bias_logits_norm']:.3f}` | `{row['memory_gated_keyed_route_bias_logits_norm']:.3f}` | `{row['memory_base_route_bias_entropy']:.3f}` | `{row['memory_keyed_residual_gate_mean']:.3f}` | `{row['run_dir']}` |"
    )
