...
ROOT="outputs/experiments/memory_context_gain_probe"
REPORT_MD="outputs/reports/memory_context_gain_probe.md"
REPORT_JSON="outputs/reports/memory_context_gain_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_context_gain.yaml"
  "configs/experiments/minigrid_memory_por_switchy_context_gain_strong.yaml"
...
manifest = {
    ...
    "family": "memory_context_gain_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_context_gain_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_context_gain",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_context_gain_probe/memory_por_switchy_context_gain"),
        "variant": "scale_0.5",
    },
    {
        "label": "por_context_gain_strong",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_context_gain_probe/memory_por_switchy_context_gain_strong"),
        "variant": "scale_1.0",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_context_gain_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_context_gain_stability": float(train_metrics.get("policy/option_context_gain_stability", 0.0)),
            "policy/option_context_gain_mean": float(train_metrics.get("policy/option_context_gain_mean", 0.0)),
            "policy/option_context_gain_proj_norm": float(train_metrics.get("policy/option_context_gain_proj_norm", 0.0)),
        }
...
por_gain = row("por_context_gain")
por_gain_strong = row("por_context_gain_strong")
gru_base = row("gru_base")

lines = [
    "# Memory Context-Gain Probe",
    "",
    "- hypothesis: amplifying the existing POR option-context projection only when persistence is stable can improve Memory decode with less disruption than new actor heads",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Context Stability | Context Gain Mean |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_gain, por_gain_strong, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_context_gain_stability']:.4f}` | `{item['policy/option_context_gain_mean']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_context_gain` | `{por_gain['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_gain['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_gain['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_gain['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_context_gain_strong` | `{por_gain_strong['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_gain_strong['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_gain_strong['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_gain_strong['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a checkpoint reuse sweep.",
        "- Context gain only counts as interesting if it improves the sampled band or greedy decode relative to the matched POR base rerun.",
    ]
)
