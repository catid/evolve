...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_probe_run_summary.csv"
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual" in config_text:
...
lines = [
    "# Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Memory Mix | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Total Logits Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in ("sare_phase_memory", "sare_phase_memory_route_bias", "sare_phase_memory_route_bias_keyed_residual"):
    stats = variants[label]
    if not stats:
        continue
    lines.append(
        f"| `{label}` | `{stats['runs']:.0f}` | `{stats['eval_success_rate']:.3f}` | `{stats['eval_return']:.3f}` | `{stats['train_success_rate']:.3f}` | `{stats['throughput_fps']:.1f}` | `{stats['route_entropy']:.3f}` | `{stats['memory_mix']:.3f}` | `{stats['memory_route_bias_norm']:.3f}` | `{stats['memory_base_route_bias_logits_norm']:.3f}` | `{stats['memory_keyed_route_bias_logits_norm']:.3f}` | `{stats['memory_route_bias_logits_norm']:.3f}` |"
    )
lines.extend(
    [
        "",
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Total Logits Norm | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in rows:
    lines.append(
        f"| `{row['variant']}` | `{row['seed']}` | `{row['eval_success_rate']:.3f}` | `{row['eval_return']:.3f}` | `{row['train_success_rate']:.3f}` | `{row['throughput_fps']:.1f}` | `{row['memory_route_bias_norm']:.3f}` | `{row['memory_base_route_bias_logits_norm']:.3f}` | `{row['memory_keyed_route_bias_logits_norm']:.3f}` | `{row['memory_route_bias_logits_norm']:.3f}` | `{row['run_dir']}` |"
    )
