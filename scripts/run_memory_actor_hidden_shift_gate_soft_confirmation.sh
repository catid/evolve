...
ROOT="outputs/experiments/memory_actor_hidden_shift_gate_soft_confirmation"
REPORT_MD="outputs/reports/memory_actor_hidden_shift_gate_soft_confirmation.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_shift_gate_soft_confirmation.json"
...
declare -a RUN_LABELS=(
  ...
  "por_actor_hidden_partial_shift22_shiftgate050"
)

declare -a RUN_DIRS=(
  "outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_rerun"
  "outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22"
  "outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate075"
  "outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate050"
)
...
por_shift050 = row("por_actor_hidden_partial_shift22_shiftgate050")

lines = [
    "# Memory Actor-Hidden Shift-Gate Softening Confirmation",
    "",
    "- follow-up to the 64-episode screen in `memory_actor_hidden_shift_gate_soft_probe.md`",
    "- task: `MiniGrid-MemoryS9-v0`",
    "- evaluation episodes per mode: `256`",
    "- matched rerun roots: base POR, fresh-root `partial_shift22`, surviving `shiftgate075`, and dead contrast `shiftgate050`",
    "",
    "## Aggregate",
    "",
...
for item in (por_base, por_shift22, por_shift075, por_shift050):
    lines.append(
...
lines.extend(
    [
        ...
        "",
        f"- `shiftgate075`: greedy `{por_shift075['greedy/success'] - por_shift22['greedy/success']:+.4f}`, lower `{por_shift075['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}`, gap `{por_shift075['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}`, shoulder `{por_shift075['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}`",
        f"- `shiftgate050`: greedy `{por_shift050['greedy/success'] - por_shift22['greedy/success']:+.4f}`, lower `{por_shift050['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}`, gap `{por_shift050['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}`, shoulder `{por_shift050['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}`",
        "",
        "## Conclusion",
        "",
    ]
...
if por_shift075["greedy/success"] > 0.0 and (
    por_shift075["lower_t005/success"] > por_shift22["lower_t005/success"]
    or por_shift075["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    or por_shift075["shoulder_t008/success"] > por_shift22["shoulder_t008/success"]
):
    lines.append(
        f"- `shiftgate075` remains alive at `greedy/lower/gap/shoulder = {por_shift075['greedy/success']:.4f} / {por_shift075['lower_t005/success']:.4f} / {por_shift075['gap_t0055/success']:.4f} / {por_shift075['shoulder_t008/success']:.4f}`"
    )
    lines.append(
        "- interpretation: mildly softening only the additive shift gate restores the local greedy-conversion surface and survives beyond the 64-episode screen"
    )
else:
    lines.append(
        f"- `shiftgate075` does not hold up as a clear improvement over the fresh-root incumbent at 256 episodes: `greedy/lower/gap/shoulder = {por_shift075['greedy/success']:.4f} / {por_shift075['lower_t005/success']:.4f} / {por_shift075['gap_t0055/success']:.4f} / {por_shift075['shoulder_t008/success']:.4f}` versus incumbent `{por_shift22['greedy/success']:.4f} / {por_shift22['lower_t005/success']:.4f} / {por_shift22['gap_t0055/success']:.4f} / {por_shift22['shoulder_t008/success']:.4f}`"
    )
    lines.append(
        "- interpretation: the shift-gate softening effect is either too weak or too unstable to treat as a real local improvement"
    )
