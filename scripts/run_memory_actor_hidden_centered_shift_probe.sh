...
ROOT="outputs/experiments/memory_actor_hidden_centered_shift_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_centered_shift_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_centered_shift_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_centered_shift.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_centered_shift2x.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_centered_shift_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_centered_shift_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_centered_shift_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift_0.22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_centered_shift",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_centered_shift_probe/memory_por_switchy_actor_hidden_partial_shift22_centered_shift"),
        "variant": "centered_shift_1x",
    },
    {
        "label": "por_actor_hidden_partial_shift22_centered_shift2x",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_centered_shift_probe/memory_por_switchy_actor_hidden_partial_shift22_centered_shift2x"),
        "variant": "centered_shift_2x",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_center_shift": float(train_metrics.get("policy/option_hidden_center_shift", 0.0)),
            "policy/option_hidden_center_shift_scale": float(train_metrics.get("policy/option_hidden_center_shift_scale", 0.0)),
            "policy/option_hidden_center_shift_mean_abs": float(train_metrics.get("policy/option_hidden_center_shift_mean_abs", 0.0)),
...
por_center1 = row("por_actor_hidden_partial_shift22_centered_shift")
por_center2 = row("por_actor_hidden_partial_shift22_centered_shift2x")
...
for item in (por_base, por_shift22, por_center1, por_center2):
    lines.append(
        f"| `{item['label']}` | `{item['greedy/success']:.4f}` | `{item['lower_t005/success']:.4f}` | `{item['gap_t0055/success']:.4f}` | `{item['shoulder_t008/success']:.4f}` | "
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_center_shift']:.1f}` | `{item['policy/option_hidden_center_shift_scale']:.2f}` | "
        f"`{item['policy/option_hidden_center_shift_mean_abs']:.6f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_centered_shift` | `{por_center1['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_center1['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_center1['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_center1['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_centered_shift2x` | `{por_center2['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_center2['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_center2['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_center2['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_center1, por_center2)
)
...
if interesting:
    lines.extend(
        [
            "## Outcome",
            "",
            "At least one centered-shift variant preserved nonzero greedy conversion while improving part of the lower/gap sampled band over `partial_shift22`, so this surface should get a longer confirmation pass before being accepted or rejected.",
        ]
    )
else:
    lines.extend(
