...
ROOT="outputs/experiments/memory_actor_hidden_bounded_shift_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_bounded_shift_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_bounded_shift_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_bounded_shift.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_bounded_shift2x.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_bounded_shift_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_bounded_shift_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_bounded_shift_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift_0.22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_bounded_shift",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_bounded_shift_probe/memory_por_switchy_actor_hidden_partial_shift22_bounded_shift"),
        "variant": "bounded_shift_1x",
    },
    {
        "label": "por_actor_hidden_partial_shift22_bounded_shift2x",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_bounded_shift_probe/memory_por_switchy_actor_hidden_partial_shift22_bounded_shift2x"),
        "variant": "bounded_shift_2x",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_bound_shift": float(train_metrics.get("policy/option_hidden_bound_shift", 0.0)),
            "policy/option_hidden_shift_bound_scale": float(train_metrics.get("policy/option_hidden_shift_bound_scale", 0.0)),
...
por_bounded = row("por_actor_hidden_partial_shift22_bounded_shift")
por_bounded2x = row("por_actor_hidden_partial_shift22_bounded_shift2x")
...
for item in (por_base, por_shift22, por_bounded, por_bounded2x):
    lines.append(
        ...
        f"`{item['policy/option_hidden_bound_shift']:.1f}` | `{item['policy/option_hidden_shift_bound_scale']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_bounded_shift` | `{por_bounded['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_bounded['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_bounded['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_bounded['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_bounded_shift2x` | `{por_bounded2x['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_bounded2x['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_bounded2x['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_bounded2x['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)

interesting = (
    (por_bounded["greedy/success"] > 0.0 and (
        por_bounded["lower_t005/success"] > por_shift22["lower_t005/success"]
        or por_bounded["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    ))
    or (por_bounded2x["greedy/success"] > 0.0 and (
        por_bounded2x["lower_t005/success"] > por_shift22["lower_t005/success"]
        or por_bounded2x["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    ))
)
...
if interesting:
    lines.extend(
        [
            "## Outcome",
            "",
            "At least one bounded-shift variant preserved nonzero greedy conversion while improving part of the sampled band over `partial_shift22`, so this surface should get a longer confirmation pass before being accepted or rejected.",
        ]
    )
else:
    lines.extend(
