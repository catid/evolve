...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_weak_base_expert_gate_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_gate_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_gate_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_gate_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_weak_base_expert_gate_probe_run_summary.csv"
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_weak_base_expert_gate_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_weak_base_expert_gate" in config_text:
...
lines = [
    "# Weak-Base Expert-Gated Keyed-Residual Route-Bias Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Gated Keyed Norm | Weak-Base Gate Mean | Weak-Base Gate Std | Keyed Scale Mean | Total Logits Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label, stats in variants.items():
    lines.append(
        "| {label} | {runs:.0f} | {eval_success_rate:.3f} | {eval_return:.3f} | {train_success_rate:.3f} | {throughput_fps:.1f} | {route_entropy:.3f} | {memory_route_bias_norm:.3f} | {memory_base_route_bias_logits_norm:.3f} | {memory_keyed_route_bias_logits_norm:.3f} | {memory_gated_keyed_route_bias_logits_norm:.3f} | {memory_weak_base_expert_gate_mean:.3f} | {memory_weak_base_expert_gate_std:.3f} | {memory_keyed_residual_scale_mean:.3f} | {memory_route_bias_logits_norm:.3f} |".format(
            label=label,
            **stats,
        )
    )
