...
ROOT="outputs/experiments/memory_actor_hidden_adaptive_shift_floor_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_adaptive_shift_floor_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_adaptive_shift_floor_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftfloor16.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftfloor18.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_adaptive_shift_floor_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_adaptive_shift_floor_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_adaptive_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftfloor16",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_adaptive_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftfloor16"),
        "variant": "shift_floor_0.16",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftfloor18",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_adaptive_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftfloor18"),
        "variant": "shift_floor_0.18",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_adaptive_shift_weight_mean": float(
                train_metrics.get("policy/option_hidden_adaptive_shift_weight_mean", 0.0)
            ),
...
por_floor16 = row("por_actor_hidden_partial_shift22_shiftfloor16")
por_floor18 = row("por_actor_hidden_partial_shift22_shiftfloor18")

lines = [
    "# Memory Actor-Hidden Adaptive Shift-Floor Probe",
    "",
    "- hypothesis: `partial_shift22` may be over-applying the additive shift on lower-duration states; a duration-conditioned shift floor could preserve the greedy conversion while widening the sampled band without touching the multiplicative scale branch",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Gate Signal | Gate Mean | Adaptive Shift Floor | Floor Value | Floor Power | Adaptive Shift Weight Mean | Scale Norm | Shift Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_shift22, por_floor16, por_floor18):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | "
        f"`{item['policy/option_hidden_adaptive_shift_floor']:.0f}` | `{item['policy/option_hidden_shift_floor']:.2f}` | `{item['policy/option_hidden_shift_floor_power']:.2f}` | "
        f"`{item['policy/option_hidden_adaptive_shift_weight_mean']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_shiftfloor16` | `{por_floor16['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_floor16['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_floor16['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_floor16['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_shiftfloor18` | `{por_floor18['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_floor18['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_floor18['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_floor18['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    and (
        item["greedy/success"] > por_shift22["greedy/success"]
        or item["lower_t005/success"] > por_shift22["lower_t005/success"]
        or item["gap_t0055/success"] > por_shift22["gap_t0055/success"]
        or item["shoulder_t008/success"] > por_shift22["shoulder_t008/success"]
    )
    for item in (por_floor16, por_floor18)
)

if interesting:
    lines.append("- conclusion: at least one adaptive shift-floor variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: duration-conditioned softening of the additive shift branch does not improve `partial_shift22`; this local actor-hidden surface does not want a simple adaptive shift floor")
