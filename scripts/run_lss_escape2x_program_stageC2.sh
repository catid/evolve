...
then
  ...
  ./.venv/bin/python -m psmn_rl.analysis.lss_portfolio_campaign verification-report \
...
PY
  ...
  ./.venv/bin/python -m psmn_rl.analysis.lss_portfolio_campaign fairness-report \
    --campaign-config "$CAMPAIGN_CONFIG" \
    --output "$(./.venv/bin/python - <<'PY' "$CAMPAIGN_CONFIG"
import sys
from pathlib import Path
...
./.venv/bin/python -m psmn_rl.analysis.lss_escape2x practicalization-verification-report \
  --campaign-config "$CAMPAIGN_CONFIG" \
