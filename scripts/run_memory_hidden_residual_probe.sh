...
ROOT="outputs/experiments/memory_hidden_residual_probe"
REPORT_MD="outputs/reports/memory_hidden_residual_probe.md"
REPORT_JSON="outputs/reports/memory_hidden_residual_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_hidden_residual.yaml"
  "configs/experiments/minigrid_memory_por_switchy_hidden_residual_small.yaml"
...
manifest = {
    ...
    "family": "memory_hidden_residual_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_hidden_residual_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_hidden_residual",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_hidden_residual_probe/memory_por_switchy_hidden_residual"),
        "variant": "scale_0.5",
    },
    {
        "label": "por_hidden_residual_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_hidden_residual_probe/memory_por_switchy_hidden_residual_small"),
        "variant": "scale_0.2",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_hidden_residual_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_residual_stability": float(
                train_metrics.get("policy/option_hidden_residual_stability", 0.0)
            ),
            "policy/option_hidden_residual_norm": float(train_metrics.get("policy/option_hidden_residual_norm", 0.0)),
        }
...
por_hidden_residual = row("por_hidden_residual")
por_hidden_residual_small = row("por_hidden_residual_small")
gru_base = row("gru_base")

lines = [
    "# Memory Hidden-Residual Probe",
    "",
    "- hypothesis: a POR-specific stability-gated hidden residual can improve the live Memory sampled band without the overconfident collapse caused by direct logit-space intervention",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Hidden Stability | Hidden Residual Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_hidden_residual, por_hidden_residual_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_residual_stability']:.4f}` | `{item['policy/option_hidden_residual_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_hidden_residual` | `{por_hidden_residual['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_hidden_residual['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_hidden_residual['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_hidden_residual['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_hidden_residual_small` | `{por_hidden_residual_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_hidden_residual_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_hidden_residual_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_hidden_residual_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a reused-checkpoint sweep.",
        "- The hidden residual only counts as interesting if it improves the sampled band or greedy decode relative to the matched POR base rerun.",
    ]
)
