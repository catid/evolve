...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_per_expert_scale_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_per_expert_scale_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_per_expert_scale_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_per_expert_scale_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_per_expert_scale_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_per_expert_scale_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_per_expert_scale_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_per_expert_scale" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_keyed_residual_scale_std": float(last.get("memory/keyed_residual_scale_std", 0.0)),
        }
    )
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_keyed_residual_scale_std",
    ]
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_per_expert_scale": summarize("sare_phase_memory_route_bias_keyed_residual_per_expert_scale"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Per-Expert-Scale Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_per_expert_scale`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Scaled Keyed Norm | Total Logits Norm | Scale Mean | Scale Std |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
...
lines.extend(
    [
        ...
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Route Bias Norm | Keyed Logits Norm | Scaled Keyed Norm | Total Logits Norm | Scale Mean | Scale Std | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in rows:
    lines.append(
        f"| `{row['variant']}` | `{row['seed']}` | `{row['eval_success_rate']:.3f}` | `{row['eval_return']:.3f}` | `{row['train_success_rate']:.3f}` | `{row['throughput_fps']:.1f}` | `{row['memory_route_bias_norm']:.3f}` | `{row['memory_keyed_route_bias_logits_norm']:.3f}` | `{row['memory_scaled_keyed_route_bias_logits_norm']:.3f}` | `{row['memory_route_bias_logits_norm']:.3f}` | `{row['memory_keyed_residual_scale_mean']:.3f}` | `{row['memory_keyed_residual_scale_std']:.3f}` | `{row['run_dir']}` |"
    )
