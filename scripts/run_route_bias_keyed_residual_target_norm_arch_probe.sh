...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_target_norm_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_target_norm_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_target_norm_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_target_norm_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_target_norm_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_target_norm_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_target_norm_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_target_norm" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_scaled_keyed_route_bias_logits_norm": float(last.get("memory/scaled_keyed_route_bias_logits_norm", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_scaled_keyed_route_bias_logits_norm",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_target_norm": summarize("sare_phase_memory_route_bias_keyed_residual_target_norm"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Target-Norm Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_target_norm`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Scaled Keyed Norm | Total Logits Norm | Scale Mean | Scale Min | Scale Max |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
    ...
    if not stats:
        continue
    lines.append(
        f"| `{label}` | `{stats['runs']:.0f}` | `{stats['eval_success_rate']:.3f}` | `{stats['eval_return']:.3f}` | `{stats['train_success_rate']:.3f}` | `{stats['throughput_fps']:.1f}` | `{stats['route_entropy']:.3f}` | `{stats['memory_route_bias_norm']:.3f}` | `{stats['memory_base_route_bias_logits_norm']:.3f}` | `{stats['memory_keyed_route_bias_logits_norm']:.3f}` | `{stats['memory_scaled_keyed_route_bias_logits_norm']:.3f}` | `{stats['memory_route_bias_logits_norm']:.3f}` | `{stats['memory_keyed_residual_scale_mean']:.3f}` | `{stats['memory_keyed_residual_scale_min']:.3f}` | `{stats['memory_keyed_residual_scale_max']:.3f}` |"
    )
lines.extend(
    [
        "",
