...
ROOT="outputs/experiments/memory_actor_hidden_branch_gate_bias_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_branch_gate_bias_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_branch_gate_bias_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_scalebias10.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftbias10.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_branch_gate_bias_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_branch_gate_bias_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_branch_gate_bias_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_scalebias10",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_branch_gate_bias_probe/memory_por_switchy_actor_hidden_partial_shift22_scalebias10"),
        "variant": "scale_bias_0.10",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftbias10",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_branch_gate_bias_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftbias10"),
        "variant": "shift_bias_0.10",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_gate_bias_scale": float(train_metrics.get("policy/option_hidden_gate_bias_scale", 0.0)),
...
por_scalebias10 = row("por_actor_hidden_partial_shift22_scalebias10")
por_shiftbias10 = row("por_actor_hidden_partial_shift22_shiftbias10")

lines = [
    "# Memory Actor-Hidden Branch Gate-Bias Probe",
    "",
    "- hypothesis: the shared gate-bias probe was too blunt; biasing only the scale or only the shift branch may preserve `partial_shift22` greedy conversion while recovering sampled-band headroom on `Memory`",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Gate Signal | Shared Bias | Scale Bias | Shift Bias | Gate Bias Mean | Scale Bias Mean | Shift Bias Mean | Gate Mean | Scale Norm | Shift Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_shift22, por_scalebias10, por_shiftbias10):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_gate_bias_scale']:.2f}` | "
        f"`{item['policy/option_hidden_scale_gate_bias_scale']:.2f}` | `{item['policy/option_hidden_shift_gate_bias_scale']:.2f}` | `{item['policy/option_hidden_gate_bias_mean']:.4f}` | "
        f"`{item['policy/option_hidden_scale_gate_bias_mean']:.4f}` | `{item['policy/option_hidden_shift_gate_bias_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | "
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_scalebias10` | `{por_scalebias10['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_scalebias10['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_scalebias10['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_scalebias10['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_shiftbias10` | `{por_shiftbias10['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_shiftbias10['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_shiftbias10['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_shiftbias10['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_scalebias10, por_shiftbias10)
)

if interesting:
    lines.append("- conclusion: at least one branch-specific gate-bias variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: splitting the learned gate-bias by scale vs shift branch does not improve `partial_shift22`; this local actor-hidden surface does not benefit from branch-specific threshold shifts either")
