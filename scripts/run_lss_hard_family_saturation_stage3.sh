...
for block in definition["hard_family_holdout_blocks"]:
    lane = block["lane"]
  ...
  for lane in "${!BLOCK_SEEDS[@]}"; do
    ...
    for seed in "${seeds[@]}"; do
      for variant in sare token_dense single_expert; do
        ...
        spec_path="${STAGE3_ROOT}/${BEST_CANDIDATE}/${lane}/seed_${seed}/configs/${output_label}.yaml"
        output_dir="${STAGE3_ROOT}/${BEST_CANDIDATE}/${lane}/seed_${seed}/${output_label}"
        render_lss_spec "$CAMPAIGN_CONFIG" "$BEST_CANDIDATE" "$variant" "${BLOCK_TEACHER_ROOT[$lane]}" "$student_root" "$lane" "$seed" "$spec_path" "$output_dir"
...
python -m psmn_rl.analysis.lss_hard_family_saturation stage4-report \
