...
ROOT="outputs/experiments/memory_actor_hidden_shift_gate_soft_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_shift_gate_soft_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_shift_gate_soft_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftgate050.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_shift_gate_soft_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate075",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate075"),
        "variant": "shift_gate_power_0.75",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate050",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shift_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate050"),
        "variant": "shift_gate_power_0.50",
    },
]
...
por_gate075 = row("por_actor_hidden_partial_shift22_shiftgate075")
por_gate050 = row("por_actor_hidden_partial_shift22_shiftgate050")

lines = [
    "# Memory Actor-Hidden Shift-Gate Softening Probe",
    "",
    "- hypothesis: the actor-hidden FiLM shift branch may be too sparse under rerun-sensitive conditions; softening only the shift-branch duration gate could preserve additive intervention strength without changing the multiplicative scale branch",
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_shiftgate075` | `{por_gate075['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate075['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate075['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate075['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_shiftgate050` | `{por_gate050['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate050['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate050['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate050['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
if interesting:
    lines.append("- conclusion: at least one shift-gate softening variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: softening the actor-hidden shift-branch duration gate does not improve `partial_shift22`; broadening the additive gate also fails to recover this local surface")
