...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_top1_disagreement_bonus_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_top1_disagreement_bonus" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_disagreement_bonus_logits_norm": float(last.get("memory/disagreement_bonus_logits_norm", 0.0)),
            "memory_keyed_route_bias_logits_norm": float(last.get("memory/keyed_route_bias_logits_norm", 0.0)),
            "memory_top1_disagreement_rate": float(last.get("memory/top1_disagreement_rate", 0.0)),
            "memory_top1_disagreement_density": float(last.get("memory/top1_disagreement_density", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_disagreement_bonus_logits_norm",
        "memory_keyed_route_bias_logits_norm",
        "memory_top1_disagreement_rate",
        "memory_top1_disagreement_density",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_top1_disagreement_bonus": summarize("sare_phase_memory_route_bias_keyed_residual_predictive_top1_disagreement_bonus"),
}

report_json.write_text(json.dumps({"runs": rows, "summary": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Top-1 Disagreement-Bonus Keyed-Residual Route-Bias Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Prior Keyed Norm | Predictive Keyed Norm | Disagreement Bonus Norm | Final Keyed Norm | Top1 Disagreement Rate | Top1 Disagreement Density | Total Logits Norm | Predictive Hidden Norm | Predictive Delta Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
