...
ROOT="outputs/experiments/memory_top2_rerank_probe"
REPORT_MD="outputs/reports/memory_top2_rerank_probe.md"
REPORT_JSON="outputs/reports/memory_top2_rerank_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_top2_rerank.yaml"
  "configs/experiments/minigrid_memory_token_gru_long_rerun.yaml"
  "configs/experiments/minigrid_memory_token_gru_long_top2_rerank.yaml"
)
...
manifest = {
    ...
    "family": "memory_top2_rerank_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_top2_rerank_probe/memory_por_switchy_rerun"),
        "top2_rerank": False,
    },
    {
        "label": "por_top2_rerank",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_top2_rerank_probe/memory_por_switchy_top2_rerank"),
        "top2_rerank": True,
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_top2_rerank_probe/memory_token_gru_long_rerun"),
        "top2_rerank": False,
    },
    {
        "label": "gru_top2_rerank",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_top2_rerank_probe/memory_token_gru_long_top2_rerank"),
        "top2_rerank": True,
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/top2_rerank_gate_mean": float(train_metrics.get("policy/top2_rerank_gate_mean", 0.0)),
            "policy/top2_rerank_low_margin_mean": float(train_metrics.get("policy/top2_rerank_low_margin_mean", 0.0)),
            "policy/top2_rerank_raw_delta_mean_abs": float(train_metrics.get("policy/top2_rerank_raw_delta_mean_abs", 0.0)),
            "policy/top2_rerank_effective_delta_mean_abs": float(train_metrics.get("policy/top2_rerank_effective_delta_mean_abs", 0.0)),
        }
...
por_rerank = find_row("por_top2_rerank")
gru_base = find_row("gru_base")
gru_rerank = find_row("gru_top2_rerank")

lines = [
    "# Memory Top2 Rerank Probe",
    "",
    "- hypothesis: a targeted low-margin top-2 reranker can repair top-choice ordering on Memory with less disruption than broad residual heads",
    ...
    "## Aggregate",
    "",
    "| Family | Base Greedy | Rerank Greedy | Base Gap t0.055 | Rerank Gap t0.055 | Base Shoulder t0.08 | Rerank Shoulder t0.08 | Delta Greedy | Delta Gap | Delta Shoulder |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    f"| `por` | `{por_base['greedy/success']:.4f}` | `{por_rerank['greedy/success']:.4f}` | `{por_base['gap_t0055/success']:.4f}` | `{por_rerank['gap_t0055/success']:.4f}` | `{por_base['shoulder_t008/success']:.4f}` | `{por_rerank['shoulder_t008/success']:.4f}` | `{por_rerank['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_rerank['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_rerank['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
    f"| `token_gru` | `{gru_base['greedy/success']:.4f}` | `{gru_rerank['greedy/success']:.4f}` | `{gru_base['gap_t0055/success']:.4f}` | `{gru_rerank['gap_t0055/success']:.4f}` | `{gru_base['shoulder_t008/success']:.4f}` | `{gru_rerank['shoulder_t008/success']:.4f}` | `{gru_rerank['greedy/success'] - gru_base['greedy/success']:+.4f}` | `{gru_rerank['gap_t0055/success'] - gru_base['gap_t0055/success']:+.4f}` | `{gru_rerank['shoulder_t008/success'] - gru_base['shoulder_t008/success']:+.4f}` |",
    "",
    "## Per Run",
    "",
    "| Label | Family | Top2 Rerank | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Gate Mean | Low-Margin Mean | Raw Delta | Effective Delta | Throughput |",
    "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]

for row in rows:
    lines.append(
        f"| `{row['label']}` | `{row['family']}` | `{'yes' if row['top2_rerank'] else 'no'}` | "
        ...
        f"`{row['train/episode_return']:.4f}` | `{row['train/success_rate']:.4f}` | `{row['policy/top2_rerank_gate_mean']:.4f}` | `{row['policy/top2_rerank_low_margin_mean']:.4f}` | "
        f"`{row['policy/top2_rerank_raw_delta_mean_abs']:.4f}` | `{row['policy/top2_rerank_effective_delta_mean_abs']:.4f}` | `{row['throughput_fps']:.1f}` |"
    )
...
lines.extend(
    [
        ...
        "## Interpretation",
        "",
        "- This probe is a fresh matched base-vs-top2-rerank retrain, not a checkpoint reuse comparison.",
