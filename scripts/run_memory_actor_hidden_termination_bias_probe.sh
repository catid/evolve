...
ROOT="outputs/experiments/memory_actor_hidden_termination_bias_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_termination_bias_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_termination_bias_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_termbias075.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_termbias125.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_termination_bias_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_termination_bias_probe/memory_por_switchy_rerun"),
        "variant": "none",
        "termination_bias": 1.0,
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_termination_bias_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift_0.22",
        "termination_bias": 1.0,
    },
    {
        "label": "por_actor_hidden_partial_shift22_termbias075",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_termination_bias_probe/memory_por_switchy_actor_hidden_partial_shift22_termbias075"),
        "variant": "partial_shift_0.22_termbias_0.75",
        "termination_bias": 0.75,
    },
    {
        "label": "por_actor_hidden_partial_shift22_termbias125",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_termination_bias_probe/memory_por_switchy_actor_hidden_partial_shift22_termbias125"),
        "variant": "partial_shift_0.22_termbias_1.25",
        "termination_bias": 1.25,
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            "label": run["label"],
            "variant": run["variant"],
            "termination_bias": float(run["termination_bias"]),
            "run_dir": str(run_dir),
            ...
            "option_duration": float(train_metrics.get("policy/option_duration", 0.0)),
            "option_switch_rate": float(train_metrics.get("policy/option_switch_rate", 0.0)),
...
por_bias075 = row("por_actor_hidden_partial_shift22_termbias075")
por_bias125 = row("por_actor_hidden_partial_shift22_termbias125")
...
for item in (por_base, por_shift22, por_bias075, por_bias125):
    lines.append(
        f"| `{item['label']}` | `{item['termination_bias']:.2f}` | `{item['greedy/success']:.4f}` | `{item['lower_t005/success']:.4f}` | `{item['gap_t0055/success']:.4f}` | `{item['shoulder_t008/success']:.4f}` | "
        f"`{item['option_duration']:.4f}` | `{item['option_switch_rate']:.4f}` | `{item['greedy/margin']:.4f}` | `{item['gap_t0055/margin']:.4f}` | `{item['shoulder_t008/margin']:.4f}` | "
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['throughput_fps']:.2f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_termbias075` | `{por_bias075['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_bias075['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_bias075['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_bias075['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` | `{por_bias075['option_duration'] - por_shift22['option_duration']:+.4f}` | `{por_bias075['option_switch_rate'] - por_shift22['option_switch_rate']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_termbias125` | `{por_bias125['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_bias125['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_bias125['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_bias125['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` | `{por_bias125['option_duration'] - por_shift22['option_duration']:+.4f}` | `{por_bias125['option_switch_rate'] - por_shift22['option_switch_rate']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_bias075, por_bias125)
)

if interesting:
    lines.append("- conclusion: at least one termination-bias variant preserves the greedy branch and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: mild `termination_bias` shifts do not improve on `partial_shift22`; this local greedy-conversion surface does not want a simple persistence-bias tweak")
