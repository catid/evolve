...
ROOT="outputs/experiments/memory_actor_hidden_scale_gate_power_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_scale_gate_power_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_scale_gate_power_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_scalegate125.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_scalegate150.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_scale_gate_power_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_power_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_power_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_scalegate125",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_power_probe/memory_por_switchy_actor_hidden_partial_shift22_scalegate125"),
        "variant": "scale_gate_power_1.25",
    },
    {
        "label": "por_actor_hidden_partial_shift22_scalegate150",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_power_probe/memory_por_switchy_actor_hidden_partial_shift22_scalegate150"),
        "variant": "scale_gate_power_1.50",
    },
]
...
por_gate125 = row("por_actor_hidden_partial_shift22_scalegate125")
por_gate150 = row("por_actor_hidden_partial_shift22_scalegate150")

lines = [
    "# Memory Actor-Hidden Scale-Gate Power Probe",
    "",
    "- hypothesis: the actor-hidden FiLM scale branch may be too diffuse on middling duration states; sharpening only the scale-branch duration gate could make the multiplicative intervention more selective while leaving the additive branch unchanged",
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_scalegate125` | `{por_gate125['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate125['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate125['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate125['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_scalegate150` | `{por_gate150['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate150['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate150['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate150['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
if interesting:
    lines.append("- conclusion: at least one scale-gate power variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: sharpening the actor-hidden scale-branch duration gate does not improve `partial_shift22`; this local surface does not benefit from a more selective multiplicative gate either")
