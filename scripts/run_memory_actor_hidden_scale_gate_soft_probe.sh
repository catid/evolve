...
ROOT="outputs/experiments/memory_actor_hidden_scale_gate_soft_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_scale_gate_soft_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_scale_gate_soft_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_scalegate075.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_scalegate050.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_scale_gate_soft_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_soft_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_scalegate075",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22_scalegate075"),
        "variant": "scale_gate_power_0.75",
    },
    {
        "label": "por_actor_hidden_partial_shift22_scalegate050",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_scale_gate_soft_probe/memory_por_switchy_actor_hidden_partial_shift22_scalegate050"),
        "variant": "scale_gate_power_0.50",
    },
]
...
por_gate075 = row("por_actor_hidden_partial_shift22_scalegate075")
por_gate050 = row("por_actor_hidden_partial_shift22_scalegate050")

lines = [
    "# Memory Actor-Hidden Scale-Gate Softening Probe",
    "",
    "- hypothesis: the actor-hidden FiLM scale branch may be too sparse under rerun-sensitive conditions; softening only the scale-branch duration gate could preserve multiplicative intervention strength without changing the additive branch",
...
for item in (por_base, por_shift22, por_gate075, por_gate050):
    lines.append(
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_scalegate075` | `{por_gate075['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate075['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate075['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate075['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_scalegate050` | `{por_gate050['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gate050['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gate050['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gate050['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_gate075, por_gate050)
)

if interesting:
    lines.append("- conclusion: at least one scale-gate softening variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: softening the actor-hidden scale-branch duration gate does not improve `partial_shift22`; broadening the multiplicative gate also fails to recover this local surface")
