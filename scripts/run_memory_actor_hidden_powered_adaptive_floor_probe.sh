...
ROOT="outputs/experiments/memory_actor_hidden_powered_adaptive_floor_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_powered_adaptive_floor_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_powered_adaptive_floor_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor25_power075.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor25_power05.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_powered_adaptive_floor_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {"label": "por_base", "run_dir": Path("outputs/experiments/memory_actor_hidden_powered_adaptive_floor_probe/memory_por_switchy_rerun")},
    {"label": "partial_shift22", "run_dir": Path("outputs/experiments/memory_actor_hidden_powered_adaptive_floor_probe/memory_por_switchy_actor_hidden_partial_shift22")},
    {"label": "adaptive_floor25", "run_dir": Path("outputs/experiments/memory_actor_hidden_powered_adaptive_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor25")},
    {"label": "adaptive_floor25_power075", "run_dir": Path("outputs/experiments/memory_actor_hidden_powered_adaptive_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor25_power075")},
    {"label": "adaptive_floor25_power05", "run_dir": Path("outputs/experiments/memory_actor_hidden_powered_adaptive_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor25_power05")},
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "scale_floor_power": float(train_metrics.get("policy/option_hidden_scale_floor_power", 0.0)),
...
for item in rows:
    lines.append(
        f"| `{item['label']}` | `{item['scale_floor_power']:.2f}` | `{item['greedy/success']:.4f}` | `{item['lower_t005/success']:.4f}` | `{item['gap_t0055/success']:.4f}` | `{item['shoulder_t008/success']:.4f}` | "
        f"`{item['greedy/margin']:.4f}` | `{item['gap_t0055/margin']:.4f}` | `{item['shoulder_t008/margin']:.4f}` | "
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['adaptive_scale_mean']:.4f}` | `{item['scale_norm']:.4f}` | `{item['shift_norm']:.4f}` |"
    )
...
best = max(rows[3:], key=lambda item: (item["greedy/success"] > 0.0, item["gap_t0055/success"], item["shoulder_t008/success"]))
lines.extend(["", "## Conclusion", ""])
if best["greedy/success"] > base["greedy/success"] and (
    ...
    lines.append(f"- `{best['label']}` improves the adaptive-floor trade-off and should get a 256-episode confirmation pass")
else:
    lines.append("- no powered floor variant improves the adaptive-floor trade-off enough to justify a confirmation pass")
