...
ROOT="outputs/experiments/memory_action_experts_probe"
REPORT_MD="outputs/reports/memory_action_experts_probe.md"
REPORT_JSON="outputs/reports/memory_action_experts_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_action_experts.yaml"
  "configs/experiments/minigrid_memory_por_switchy_action_experts_small.yaml"
...
manifest = {
    ...
    "family": "memory_action_experts_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_action_experts_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_action_experts",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_action_experts_probe/memory_por_switchy_action_experts"),
        "variant": "scale_1.0",
    },
    {
        "label": "por_action_experts_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_action_experts_probe/memory_por_switchy_action_experts_small"),
        "variant": "scale_0.3",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_action_experts_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_action_experts_stability": float(
                train_metrics.get("policy/option_action_experts_stability", 0.0)
            ),
            "policy/option_action_experts_bias_norm": float(
                train_metrics.get("policy/option_action_experts_bias_norm", 0.0)
            ),
        }
...
por_action_experts = row("por_action_experts")
por_action_experts_small = row("por_action_experts_small")
gru_base = row("gru_base")

lines = [
    "# Memory Action-Experts Probe",
    "",
    "- hypothesis: a POR-specific mixture of option-conditioned action heads can translate the live option state into better action selection than the shared policy head",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Expert Stability | Expert Bias Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_action_experts, por_action_experts_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_action_experts_stability']:.4f}` | `{item['policy/option_action_experts_bias_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_action_experts` | `{por_action_experts['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_action_experts['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_action_experts['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_action_experts['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_action_experts_small` | `{por_action_experts_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_action_experts_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_action_experts_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_action_experts_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a reused-checkpoint sweep.",
        "- The action experts only count as interesting if they improve the sampled band or greedy decode relative to the matched POR base rerun.",
    ]
)
