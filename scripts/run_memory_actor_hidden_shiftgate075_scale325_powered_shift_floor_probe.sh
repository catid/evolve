...
ROOT="outputs/experiments/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe.json"
...
manifest = {
    ...
    "family": "memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate075_scale325",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate075_scale325"),
        "variant": "shift22_shiftgate075_scale325",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate075_scale325_shiftfloor20_power075",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate075_scale325_shiftfloor20_power075"),
        "variant": "shift22_shiftgate075_scale325_shiftfloor20_power075",
    },
    {
        "label": "por_actor_hidden_partial_shift22_shiftgate075_scale325_shiftfloor20_power05",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_shiftgate075_scale325_powered_shift_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_shiftgate075_scale325_shiftfloor20_power05"),
        "variant": "shift22_shiftgate075_scale325_shiftfloor20_power05",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_adaptive_shift_floor": float(train_metrics.get("policy/option_hidden_adaptive_shift_floor", 0.0)),
            "policy/option_hidden_shift_floor": float(train_metrics.get("policy/option_hidden_shift_floor", 0.0)),
            "policy/option_hidden_shift_floor_power": float(train_metrics.get("policy/option_hidden_shift_floor_power", 0.0)),
            "policy/option_hidden_adaptive_shift_weight_mean": float(train_metrics.get("policy/option_hidden_adaptive_shift_weight_mean", 0.0)),
...
power075 = row("por_actor_hidden_partial_shift22_shiftgate075_scale325_shiftfloor20_power075")
power05 = row("por_actor_hidden_partial_shift22_shiftgate075_scale325_shiftfloor20_power05")

lines = [
    "# Memory ShiftGate075 Scale325 Powered Shift-Floor Probe",
    "",
    "- hypothesis: the plain `shiftfloor20` floor may be too uniformly applied; a concave shift-floor curve could keep higher-gate states closer to the live `shiftgate075_scale325` winner while still softening low-gate states",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Gate Signal | Gate Mean | Shift Floor | Shift Floor Power | Adaptive Shift Weight Mean | Scale Norm | Shift Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]

for item in [por_base, shift22, winner, power075, power05]:
    lines.append(
        "| `{label}` | `{greedy:.4f}` | `{lower:.4f}` | `{gap:.4f}` | `{shoulder:.4f}` | `{greedy_margin:.4f}` | `{gap_margin:.4f}` | `{shoulder_margin:.4f}` | `{train_return:.4f}` | `{train_success:.4f}` | `{gate_signal:.4f}` | `{gate_mean:.4f}` | `{shift_floor:.4f}` | `{shift_floor_power:.2f}` | `{adaptive_shift_weight_mean:.4f}` | `{scale_norm:.4f}` | `{shift_norm:.4f}` |".format(
            label=item["label"],
            greedy=item["greedy/success"],
            lower=item["lower_t005/success"],
            gap=item["gap_t0055/success"],
            shoulder=item["shoulder_t008/success"],
            greedy_margin=item["greedy/margin"],
            gap_margin=item["gap_t0055/margin"],
            shoulder_margin=item["shoulder_t008/margin"],
            train_return=item["train/episode_return"],
            train_success=item["train/success_rate"],
            gate_signal=item["policy/option_hidden_film_gate_signal_mean"],
            gate_mean=item["policy/option_hidden_film_gate_mean"],
            shift_floor=item["policy/option_hidden_shift_floor"],
            shift_floor_power=item["policy/option_hidden_shift_floor_power"],
            adaptive_shift_weight_mean=item["policy/option_hidden_adaptive_shift_weight_mean"],
...
lines.extend(
    [
        ...
        "",
        "- `shiftfloor20_power075`: greedy {g1:+.4f}, lower {l1:+.4f}, gap {gp1:+.4f}, shoulder {s1:+.4f}".format(
            g1=power075["greedy/success"] - winner["greedy/success"],
            l1=power075["lower_t005/success"] - winner["lower_t005/success"],
            gp1=power075["gap_t0055/success"] - winner["gap_t0055/success"],
            s1=power075["shoulder_t008/success"] - winner["shoulder_t008/success"],
        ),
        "- `shiftfloor20_power05`: greedy {g2:+.4f}, lower {l2:+.4f}, gap {gp2:+.4f}, shoulder {s2:+.4f}".format(
            g2=power05["greedy/success"] - winner["greedy/success"],
            l2=power05["lower_t005/success"] - winner["lower_t005/success"],
            gp2=power05["gap_t0055/success"] - winner["gap_t0055/success"],
            s2=power05["shoulder_t008/success"] - winner["shoulder_t008/success"],
        ),
        "",
        "## Conclusion",
        "",
...
if survivors:
    ...
    lines.append(
        "- powered shift floors around `shiftgate075_scale325` do not improve the current local surface enough to justify a 256-episode confirmation pass"
    )
