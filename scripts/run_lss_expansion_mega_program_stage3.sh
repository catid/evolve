...
for root in campaign.get("reuse_roots", {}).get("stage3_controls", []):
    print(f"reuse:{root}")
PY
)
...
PY
        ...
        if [[ ! -e "${output_dir}" ]]; then
          for reuse_root in "${REUSE_ROOTS[@]}"; do
            reuse_dir="${reuse_root}/${candidate}/${lane}/seed_${seed}/${output_label}"
...
python -m psmn_rl.analysis.lss_successor_migration stage2-report \
