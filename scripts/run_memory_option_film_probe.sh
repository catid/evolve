...
ROOT="outputs/experiments/memory_option_film_probe"
REPORT_MD="outputs/reports/memory_option_film_probe.md"
REPORT_JSON="outputs/reports/memory_option_film_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_option_film_small.yaml"
...
manifest = {
    ...
    "family": "memory_option_film_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_film_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_option_film",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_film_probe/memory_por_switchy_option_film"),
        "variant": "scale_0.5",
    },
    {
        "label": "por_option_film_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_film_probe/memory_por_switchy_option_film_small"),
        "variant": "scale_0.2",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_option_film_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
por_option_film_small = row("por_option_film_small")
gru_base = row("gru_base")

lines = [
    "# Memory Option-FiLM Probe",
    "",
    "- hypothesis: a POR-specific option-conditioned FiLM on the pooled policy state can preserve the live sampled Memory band without the collapse caused by direct action-space interventions",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | FiLM Stability | FiLM Scale Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_option_film, por_option_film_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_film_stability']:.4f}` | `{item['policy/option_film_scale_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_option_film` | `{por_option_film['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_option_film['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_option_film['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_option_film['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_option_film_small` | `{por_option_film_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_option_film_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_option_film_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_option_film_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a reused-checkpoint sweep.",
        "- The FiLM modulation only counts as interesting if it improves the sampled band or greedy decode relative to the matched POR base rerun.",
    ]
)
