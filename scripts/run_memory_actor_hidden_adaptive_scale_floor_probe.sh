...
ROOT="outputs/experiments/memory_actor_hidden_adaptive_scale_floor_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_adaptive_scale_floor_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_adaptive_scale_floor_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor30.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_adaptive_scale_floor_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    ...
    },
    {
        ...
        "variant": "partial_shift22_adaptive_floor25",
    },
    {
        "label": "por_actor_hidden_partial_shift22_adaptive_floor30",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_adaptive_scale_floor_probe/memory_por_switchy_actor_hidden_partial_shift22_adaptive_floor30"),
        "variant": "partial_shift22_adaptive_floor30",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_adaptive_scale_floor": float(train_metrics.get("policy/option_hidden_adaptive_scale_floor", 0.0)),
...
por_floor30 = row("por_actor_hidden_partial_shift22_adaptive_floor30")
...
for item in (por_base, por_shift22, por_floor25, por_floor30):
    lines.append(
        ...
        f"`{item['policy/option_hidden_adaptive_scale_floor']:.0f}` | `{item['policy/option_hidden_scale_floor']:.2f}` | `{item['policy/option_hidden_adaptive_scale_mean']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_adaptive_floor25` | `{por_floor25['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_floor25['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_floor25['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_floor25['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_adaptive_floor30` | `{por_floor30['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_floor30['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_floor30['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_floor30['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_floor25, por_floor30)
)

if interesting:
    lines.append("- conclusion: at least one adaptive scale-floor variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: the adaptive scale-floor variants do not improve on `partial_shift22`; this local greedy-conversion surface does not want a simple duration-conditioned softening of the scale branch")
