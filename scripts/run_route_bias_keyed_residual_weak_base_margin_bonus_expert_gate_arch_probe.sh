...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_margin_bonus_logits_norm": float(last.get("memory/margin_bonus_logits_norm", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_margin_bonus_logits_norm",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate": summarize("sare_phase_memory_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate"),
}

report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Weak-Base Margin-Bonus Expert-Gated Keyed-Residual Route-Bias Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_weak_base_expert_gate`, `sare_phase_memory_route_bias_keyed_residual_weak_base_margin_bonus_expert_gate`",
    "- env: `MiniGrid-DoorKey-5x5-v0`",
    "- seeds: `7`, `11`",
    "- PPO budget: `24` updates, `64` rollout steps, `8` train envs",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Best Train Return | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Effective Keyed Norm | Weak-Base Gate Mean | Margin Gate Mean | Combined Gate Mean | Margin Bonus Norm | Total Logits Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label, stats in variants.items():
    lines.append(
        "| {label} | {runs:.0f} | {eval_success_rate:.3f} | {eval_return:.3f} | {train_success_rate:.3f} | {best_train_return:.3f} | {throughput_fps:.1f} | {route_entropy:.3f} | {memory_route_bias_norm:.3f} | {memory_base_route_bias_logits_norm:.3f} | {memory_keyed_route_bias_logits_norm:.3f} | {memory_gated_keyed_route_bias_logits_norm:.3f} | {memory_weak_base_expert_gate_mean:.3f} | {memory_margin_advantage_gate_mean:.3f} | {memory_combined_weak_margin_advantage_gate_mean:.3f} | {memory_margin_bonus_logits_norm:.3f} | {memory_route_bias_logits_norm:.3f} |".format(
            label=label,
            **stats,
        )
    )
