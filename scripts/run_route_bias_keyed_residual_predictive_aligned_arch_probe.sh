...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_aligned_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_aligned_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_aligned_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_aligned_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_aligned_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_aligned_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_aligned_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_aligned" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_aligned_predictive_delta_logits_norm": float(last.get("memory/aligned_predictive_delta_logits_norm", 0.0)),
            "memory_aligned_predictive_delta_fraction": float(last.get("memory/aligned_predictive_delta_fraction", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_aligned_predictive_delta_logits_norm",
        "memory_aligned_predictive_delta_fraction",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_aligned": summarize("sare_phase_memory_route_bias_keyed_residual_predictive_aligned"),
}

report_json.write_text(json.dumps({"runs": rows, "summary": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Aligned Predictive Keyed-Residual Route-Bias Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Prior Keyed Norm | Predictive Keyed Norm | Final Keyed Norm | Aligned Predictive Delta Norm | Aligned Predictive Delta Fraction | Total Logits Norm | Predictive Hidden Norm | Predictive Delta Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
