...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_weak_prior_replace_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_weak_prior_replace_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_weak_prior_replace_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_weak_prior_replace_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_weak_prior_replace_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_weak_prior_replace_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_weak_prior_replace_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_weak_prior_replace" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_replaced_keyed_route_bias_logits_norm": float(last.get("memory/replaced_keyed_route_bias_logits_norm", 0.0)),
            "memory_kept_prior_keyed_route_bias_logits_norm": float(last.get("memory/kept_prior_keyed_route_bias_logits_norm", 0.0)),
            "memory_keyed_route_bias_logits_norm": float(last.get("memory/keyed_route_bias_logits_norm", 0.0)),
            "memory_weak_prior_replace_gate": float(last.get("memory/weak_prior_replace_gate", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_replaced_keyed_route_bias_logits_norm",
        "memory_kept_prior_keyed_route_bias_logits_norm",
        "memory_keyed_route_bias_logits_norm",
        "memory_weak_prior_replace_gate",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_weak_prior_replace": summarize("sare_phase_memory_route_bias_keyed_residual_predictive_weak_prior_replace"),
}

report_json.write_text(json.dumps({"runs": rows, "summary": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Weak-Prior Predictive Replacement Keyed-Residual Route-Bias Probe",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Prior Keyed Norm | Predictive Keyed Norm | Replaced Keyed Norm | Kept Prior Keyed Norm | Final Keyed Norm | Replace Gate | Total Logits Norm | Predictive Hidden Norm | Predictive Delta Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
