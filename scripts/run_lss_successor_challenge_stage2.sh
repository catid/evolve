...
for key in ("hard_dev", "hard_holdout"):
    for block in campaign["blocks"][key]:
        print(f"block:{block['lane']}:{','.join(str(seed) for seed in block['seeds'])}")
print(f"candidate:{stage1['selected_challenger']}")
PY
)
...
source .venv/bin/activate
python -m psmn_rl.analysis.lss_successor_challenge stage2-report \
