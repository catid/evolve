...
for block in campaign["seed_groups"]["healthy_blocks"]["blocks"]:
...
BEST_CANDIDATE="$(
  source .venv/bin/activate
  python - "$STAGE3_JSON" "$STAGE4_JSON" <<'PY'
import json
import sys
from pathlib import Path
stage3 = json.loads(Path(sys.argv[1]).read_text())
stage4 = json.loads(Path(sys.argv[2]).read_text())
print(stage3.get("best_candidate") if stage4.get("stage4_pass") else "")
PY
)"
  ...
  for lane in "${!BLOCK_SEEDS[@]}"; do
    ...
    for seed in "${seeds[@]}"; do
      for variant in sare token_dense single_expert; do
        ...
        spec_path="${STAGE5_ROOT}/${BEST_CANDIDATE}/${lane}/seed_${seed}/configs/${output_label}.yaml"
        output_dir="${STAGE5_ROOT}/${BEST_CANDIDATE}/${lane}/seed_${seed}/${output_label}"
...
python -m psmn_rl.analysis.lss_hard_family_program stage5-report \
