...
ROOT="outputs/experiments/memory_option_top2_rerank_probe"
REPORT_MD="outputs/reports/memory_option_top2_rerank_probe.md"
REPORT_JSON="outputs/reports/memory_option_top2_rerank_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_option_top2_rerank.yaml"
  "configs/experiments/minigrid_memory_por_switchy_option_top2_rerank_small.yaml"
...
manifest = {
    ...
    "family": "memory_option_top2_rerank_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_top2_rerank_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_option_top2_rerank",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_top2_rerank_probe/memory_por_switchy_option_top2_rerank"),
        "variant": "scale_0.5",
    },
    {
        "label": "por_option_top2_rerank_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_option_top2_rerank_probe/memory_por_switchy_option_top2_rerank_small"),
        "variant": "scale_0.25",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_option_top2_rerank_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_top2_rerank_raw_delta_mean_abs": float(train_metrics.get("policy/option_top2_rerank_raw_delta_mean_abs", 0.0)),
...
por_top2 = row("por_option_top2_rerank")
por_top2_small = row("por_option_top2_rerank_small")
gru_base = row("gru_base")

lines = [
    "# Memory Option-Aware Top2 Rerank Probe",
    "",
    "- hypothesis: a POR option-aware top2 rerank with a direct low-margin x option-stability gate can preserve the live sampled Memory band better than unconditional POR-side bias or actor-side learned gates",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Top2 Stability | Top2 Gate | Effective Delta |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_top2, por_top2_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_top2_rerank_stability']:.4f}` | `{item['policy/option_top2_rerank_gate_mean']:.4f}` | `{item['policy/option_top2_rerank_effective_delta_mean_abs']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_option_top2_rerank` | `{por_top2['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_top2['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_top2['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_top2['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_option_top2_rerank_small` | `{por_top2_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_top2_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_top2_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_top2_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a checkpoint reuse sweep.",
        "- The branch only counts as interesting if direct option-aware top2 reranking preserves or improves the live sampled band relative to the matched POR base rerun.",
    ]
)
