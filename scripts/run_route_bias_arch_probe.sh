...
ROOT="outputs/diagnostics/architecture_route_bias_probe"
REPORT_MD="outputs/reports/architecture_route_bias_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_probe_run_summary.csv"
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias" in config_text:
...
route_bias = summarize("sare_phase_memory_route_bias")
payload = {"rows": rows, "baseline": baseline, "route_bias": route_bias}
report_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Route-Biased Phase-Memory Architecture Probe",
    "",
    "- variants: `sare_phase_memory` vs `sare_phase_memory_route_bias`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Memory Mix | Route Bias Norm | Route Bias AbsMax |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label, stats in (("sare_phase_memory", baseline), ("sare_phase_memory_route_bias", route_bias)):
    if not stats:
        continue
    lines.append(
        f"| `{label}` | `{stats['runs']:.0f}` | `{stats['eval_success_rate']:.3f}` | `{stats['eval_return']:.3f}` | `{stats['train_success_rate']:.3f}` | `{stats['throughput_fps']:.1f}` | `{stats['route_entropy']:.3f}` | `{stats['memory_mix']:.3f}` | `{stats['memory_route_bias_norm']:.3f}` | `{stats['memory_route_bias_absmax']:.3f}` |"
    )
lines.extend(
    [
        "",
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Memory Mix | Route Bias Norm | Route Bias AbsMax | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in rows:
    lines.append(
        f"| `{row['variant']}` | `{row['seed']}` | `{row['eval_success_rate']:.3f}` | `{row['eval_return']:.3f}` | `{row['train_success_rate']:.3f}` | `{row['throughput_fps']:.1f}` | `{row['memory_mix']:.3f}` | `{row['memory_route_bias_norm']:.3f}` | `{row['memory_route_bias_absmax']:.3f}` | `{row['run_dir']}` |"
    )
