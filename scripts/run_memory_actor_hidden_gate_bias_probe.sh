...
ROOT="outputs/experiments/memory_actor_hidden_gate_bias_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_gate_bias_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_gate_bias_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_gatebias10.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_gatebias20.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_gate_bias_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_gate_bias_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_gate_bias_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_gatebias10",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_gate_bias_probe/memory_por_switchy_actor_hidden_partial_shift22_gatebias10"),
        "variant": "gatebias_0.10",
    },
    {
        "label": "por_actor_hidden_partial_shift22_gatebias20",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_gate_bias_probe/memory_por_switchy_actor_hidden_partial_shift22_gatebias20"),
        "variant": "gatebias_0.20",
    },
]
...
por_gatebias10 = row("por_actor_hidden_partial_shift22_gatebias10")
por_gatebias20 = row("por_actor_hidden_partial_shift22_gatebias20")
...
for item in (por_base, por_shift22, por_gatebias10, por_gatebias20):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_gate_bias']:.0f}` | "
        f"`{item['policy/option_hidden_gate_bias_scale']:.2f}` | `{item['policy/option_hidden_gate_bias_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_gatebias10` | `{por_gatebias10['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gatebias10['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gatebias10['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gatebias10['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift22_gatebias20` | `{por_gatebias20['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_gatebias20['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_gatebias20['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_gatebias20['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)
...
interesting = any(
    ...
    for item in (por_gatebias10, por_gatebias20)
)

if interesting:
    lines.append("- conclusion: at least one gate-bias variant preserves nonzero greedy and improves part of the sampled band; run a 256-episode confirmation pass before treating it as real")
else:
    lines.append("- conclusion: the gate-bias variants do not improve on `partial_shift22`; this local actor-hidden surface does not benefit from a learned shared threshold shift on the duration gate")
