...
ROOT="outputs/experiments/memory_actor_hidden_film_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_film_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_film_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_film_small.yaml"
...
manifest = {
    ...
    "family": "memory_actor_hidden_film_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_film_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_film",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_film_probe/memory_por_switchy_actor_hidden_film"),
        "variant": "scale_0.5",
    },
    {
        "label": "por_actor_hidden_film_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_film_probe/memory_por_switchy_actor_hidden_film_small"),
        "variant": "scale_0.25",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_film_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
por_film_small = row("por_actor_hidden_film_small")
...
for item in (por_base, por_film, por_film_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        f"| `por_actor_hidden_film_small` | `{por_film_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_film_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_film_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_film_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
