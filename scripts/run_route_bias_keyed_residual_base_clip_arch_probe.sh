...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_base_clip_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_base_clip_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_base_clip_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_base_clip_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_base_clip_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_base_clip_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_base_clip_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_base_clip" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_clipped_base_route_bias_logits_norm": float(last.get("memory/clipped_base_route_bias_logits_norm", 0.0)),
            ...
            "memory_base_clip_scale_mean": float(last.get("memory/base_clip_scale_mean", 0.0)),
            "memory_base_clip_scale_min": float(last.get("memory/base_clip_scale_min", 0.0)),
            "memory_base_clip_scale_max": float(last.get("memory/base_clip_scale_max", 0.0)),
        }
    )
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_clipped_base_route_bias_logits_norm",
        "memory_keyed_route_bias_logits_norm",
        "memory_route_bias_logits_norm",
        "memory_base_clip_scale_mean",
        "memory_base_clip_scale_min",
        "memory_base_clip_scale_max",
    ]
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_base_clip": summarize("sare_phase_memory_route_bias_keyed_residual_base_clip"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Base-Clipped Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_base_clip`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Clipped Base Norm | Keyed Logits Norm | Total Logits Norm | Clip Mean | Clip Min | Clip Max |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
    ...
    if not stats:
        continue
    lines.append(
        f"| `{label}` | `{stats['runs']:.0f}` | `{stats['eval_success_rate']:.3f}` | `{stats['eval_return']:.3f}` | `{stats['train_success_rate']:.3f}` | `{stats['throughput_fps']:.1f}` | `{stats['route_entropy']:.3f}` | `{stats['memory_route_bias_norm']:.3f}` | `{stats['memory_base_route_bias_logits_norm']:.3f}` | `{stats['memory_clipped_base_route_bias_logits_norm']:.3f}` | `{stats['memory_keyed_route_bias_logits_norm']:.3f}` | `{stats['memory_route_bias_logits_norm']:.3f}` | `{stats['memory_base_clip_scale_mean']:.3f}` | `{stats['memory_base_clip_scale_min']:.3f}` | `{stats['memory_base_clip_scale_max']:.3f}` |"
    )
lines.extend(
    [
        "",
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Base Logits Norm | Clipped Base Norm | Keyed Logits Norm | Total Logits Norm | Clip Mean | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in rows:
    lines.append(
        f"| `{row['variant']}` | `{row['seed']}` | `{row['eval_success_rate']:.3f}` | `{row['eval_return']:.3f}` | `{row['train_success_rate']:.3f}` | `{row['throughput_fps']:.1f}` | `{row['memory_base_route_bias_logits_norm']:.3f}` | `{row['memory_clipped_base_route_bias_logits_norm']:.3f}` | `{row['memory_keyed_route_bias_logits_norm']:.3f}` | `{row['memory_route_bias_logits_norm']:.3f}` | `{row['memory_base_clip_scale_mean']:.3f}` | `{row['run_dir']}` |"
    )
