...
python -m psmn_rl.analysis.lss_mechanism_program state-reconciliation \
  --campaign-config "$CAMPAIGN_CONFIG" \
  --output "$STATE_OUTPUT"

python -m psmn_rl.analysis.lss_mechanism_program baseline-sync \
  --campaign-config "$CAMPAIGN_CONFIG" \
  --output "$BASELINE_OUTPUT" \
  --csv "$BASELINE_CSV" \
  --json "$BASELINE_JSON"

python -m psmn_rl.analysis.lss_mechanism_program registration \
  --campaign-config "$CAMPAIGN_CONFIG" \
