...
ROOT="outputs/diagnostics/architecture_route_bias_dual_probe"
REPORT_MD="outputs/reports/architecture_route_bias_dual_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_dual_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_dual_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_dual_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_dual_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_dual_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_dual" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_route_hidden_norm": float(last.get("memory/route_hidden_norm", 0.0)),
        }
    )
...
variants = {
    ...
    "sare_phase_memory_route_bias_dual": summarize("sare_phase_memory_route_bias_dual"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Dual-Memory Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_dual`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Memory Mix | Route Bias Norm | Route Hidden Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in ("sare_phase_memory", "sare_phase_memory_route_bias", "sare_phase_memory_route_bias_dual"):
    stats = variants[label]
    if not stats:
        continue
    lines.append(
        f"| `{label}` | `{stats['runs']:.0f}` | `{stats['eval_success_rate']:.3f}` | `{stats['eval_return']:.3f}` | `{stats['train_success_rate']:.3f}` | `{stats['throughput_fps']:.1f}` | `{stats['route_entropy']:.3f}` | `{stats['memory_mix']:.3f}` | `{stats['memory_route_bias_norm']:.3f}` | `{stats['memory_route_hidden_norm']:.3f}` |"
    )
lines.extend(
    [
        "",
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Route Bias Norm | Route Hidden Norm | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in rows:
    lines.append(
        f"| `{row['variant']}` | `{row['seed']}` | `{row['eval_success_rate']:.3f}` | `{row['eval_return']:.3f}` | `{row['train_success_rate']:.3f}` | `{row['throughput_fps']:.1f}` | `{row['memory_route_bias_norm']:.3f}` | `{row['memory_route_hidden_norm']:.3f}` | `{row['run_dir']}` |"
    )
