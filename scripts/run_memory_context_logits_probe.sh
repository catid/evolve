...
ROOT="outputs/experiments/memory_context_logits_probe"
REPORT_MD="outputs/reports/memory_context_logits_probe.md"
REPORT_JSON="outputs/reports/memory_context_logits_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_context_logits.yaml"
  "configs/experiments/minigrid_memory_por_switchy_context_logits_small.yaml"
...
manifest = {
    ...
    "family": "memory_context_logits_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_context_logits_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_context_logits",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_context_logits_probe/memory_por_switchy_context_logits"),
        "variant": "scale_0.5",
    },
    {
        "label": "por_context_logits_small",
        "family": "por",
        "run_dir": Path("outputs/experiments/memory_context_logits_probe/memory_por_switchy_context_logits_small"),
        "variant": "scale_0.25",
    },
    {
        "label": "gru_base",
        "family": "token_gru",
        "run_dir": Path("outputs/experiments/memory_context_logits_probe/memory_token_gru_long_rerun"),
        "variant": "none",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_context_logits_stability": float(train_metrics.get("policy/option_context_logits_stability", 0.0)),
            "policy/option_context_logits_duration_gate": float(train_metrics.get("policy/option_context_logits_duration_gate", 0.0)),
            "policy/option_context_logits_norm": float(train_metrics.get("policy/option_context_logits_norm", 0.0)),
            "policy/option_context_logits_bias_norm": float(train_metrics.get("policy/option_context_logits_bias_norm", 0.0)),
        }
...
por_logits = row("por_context_logits")
por_logits_small = row("por_context_logits_small")
gru_base = row("gru_base")

lines = [
    "# Memory Context-Logits Probe",
    "",
    "- hypothesis: a narrow POR option-context logits head, gated by persistence stability, can improve Memory decode without the collapse seen from broader action-head interventions",
    ...
    "## Aggregate",
    "",
    "| Label | Greedy | Lower t0.05 | Gap t0.055 | Shoulder t0.08 | Greedy Margin | Gap Margin | Shoulder Margin | Train Return | Train Success | Context Stability | Context Bias Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for item in (por_base, por_logits, por_logits_small, gru_base):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_context_logits_stability']:.4f}` | `{item['policy/option_context_logits_bias_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_context_logits` | `{por_logits['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_logits['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_logits['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_logits['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        f"| `por_context_logits_small` | `{por_logits_small['greedy/success'] - por_base['greedy/success']:+.4f}` | `{por_logits_small['lower_t005/success'] - por_base['lower_t005/success']:+.4f}` | `{por_logits_small['gap_t0055/success'] - por_base['gap_t0055/success']:+.4f}` | `{por_logits_small['shoulder_t008/success'] - por_base['shoulder_t008/success']:+.4f}` |",
        "",
        "## Interpretation",
        "",
        "- This is a fresh architecture probe, not a checkpoint reuse sweep.",
        "- The branch only counts as interesting if it improves the sampled band or greedy decode relative to the matched POR base rerun.",
    ]
)
