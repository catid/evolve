...
BEST_CANDIDATE="$(
  source .venv/bin/activate
  python - "$STAGE2_JSON" "$STAGE3_JSON" <<'PY'
import json
import sys
from pathlib import Path
stage2 = json.loads(Path(sys.argv[1]).read_text())
stage3 = json.loads(Path(sys.argv[2]).read_text())
print(stage2.get("best_candidate") if stage3.get("stage4_pass") else "")
PY
)"
...
python -m psmn_rl.analysis.lss_hard_family_saturation stage5-report \
