...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_ema_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_ema_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_ema_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_ema_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_ema_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_ema_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_ema_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_ema" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_ema_hidden_norm": float(last.get("memory/ema_hidden_norm", 0.0)),
            "memory_keyed_ema_delta_norm": float(last.get("memory/keyed_ema_delta_norm", 0.0)),
        }
    )
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_ema_hidden_norm",
        "memory_keyed_ema_delta_norm",
    ]
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_ema": summarize("sare_phase_memory_route_bias_keyed_residual_ema"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# EMA Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_ema`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Keyed Logits Norm | Total Logits Norm | EMA Hidden Norm | EMA Delta Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
    ...
    lines.append(
        ...
        + f"`{summary.get('memory_ema_hidden_norm', 0.0):.3f}` | "
        + f"`{summary.get('memory_keyed_ema_delta_norm', 0.0):.3f}` |"
    )
...
lines.extend(
    [
        ...
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | EMA Hidden Norm | EMA Delta Norm | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in sorted(rows, key=lambda item: (item["variant"], item["seed"])):
    lines.append(
        ...
        + f"`{row['memory_ema_hidden_norm']:.3f}` | "
        + f"`{row['memory_keyed_ema_delta_norm']:.3f}` | "
        + f"`{row['run_dir']}` |"
    )
