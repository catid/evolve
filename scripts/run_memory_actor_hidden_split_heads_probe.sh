...
ROOT="outputs/experiments/memory_actor_hidden_split_heads_probe"
REPORT_MD="outputs/reports/memory_actor_hidden_split_heads_probe.md"
REPORT_JSON="outputs/reports/memory_actor_hidden_split_heads_probe.json"
...
declare -a CONFIGS=(
  ...
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_split_heads.yaml"
  "configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift221_split_heads.yaml"
)
...
manifest = {
    ...
    "family": "memory_actor_hidden_split_heads_probe",
    "task": "Memory",
    "gpu": gpu,
...
RUNS = [
    {
        "label": "por_base",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_split_heads_probe/memory_por_switchy_rerun"),
        "variant": "none",
    },
    {
        "label": "por_actor_hidden_partial_shift22",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_split_heads_probe/memory_por_switchy_actor_hidden_partial_shift22"),
        "variant": "partial_shift_0.22",
    },
    {
        "label": "por_actor_hidden_partial_shift22_split_heads",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_split_heads_probe/memory_por_switchy_actor_hidden_partial_shift22_split_heads"),
        "variant": "split_heads_shift_0.22",
    },
    {
        "label": "por_actor_hidden_partial_shift221_split_heads",
        "run_dir": Path("outputs/experiments/memory_actor_hidden_split_heads_probe/memory_por_switchy_actor_hidden_partial_shift221_split_heads"),
        "variant": "split_heads_shift_0.221",
    },
]
...
try:
    for run in RUNS:
        ...
        row: dict[str, Any] = {
            ...
            "policy/option_hidden_split_heads": float(train_metrics.get("policy/option_hidden_split_heads", 0.0)),
...
por_split22 = row("por_actor_hidden_partial_shift22_split_heads")
por_split221 = row("por_actor_hidden_partial_shift221_split_heads")
...
for item in (por_base, por_shift22, por_split22, por_split221):
    lines.append(
        ...
        f"`{item['train/episode_return']:.4f}` | `{item['train/success_rate']:.4f}` | `{item['policy/option_hidden_split_heads']:.1f}` | `{item['policy/option_hidden_film_gate_signal_mean']:.4f}` | `{item['policy/option_hidden_film_gate_mean']:.4f}` | `{item['policy/option_hidden_film_scale_norm']:.4f}` | `{item['policy/option_hidden_film_shift_norm']:.4f}` |"
    )
...
lines.extend(
    [
        ...
        "| --- | ---: | ---: | ---: | ---: |",
        f"| `por_actor_hidden_partial_shift22_split_heads` | `{por_split22['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_split22['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_split22['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_split22['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        f"| `por_actor_hidden_partial_shift221_split_heads` | `{por_split221['greedy/success'] - por_shift22['greedy/success']:+.4f}` | `{por_split221['lower_t005/success'] - por_shift22['lower_t005/success']:+.4f}` | `{por_split221['gap_t0055/success'] - por_shift22['gap_t0055/success']:+.4f}` | `{por_split221['shoulder_t008/success'] - por_shift22['shoulder_t008/success']:+.4f}` |",
        "",
    ]
)

interesting = (
    (por_split22["greedy/success"] > 0.0 and (
        por_split22["lower_t005/success"] > por_shift22["lower_t005/success"]
        or por_split22["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    ))
    or (por_split221["greedy/success"] > 0.0 and (
        por_split221["lower_t005/success"] > por_shift22["lower_t005/success"]
        or por_split221["gap_t0055/success"] > por_shift22["gap_t0055/success"]
    ))
)
...
if interesting:
    lines.extend(
        [
            "## Outcome",
            "",
            "At least one split-head variant preserved nonzero greedy conversion while improving part of the sampled band over `partial_shift22`, so this surface should get a longer confirmation pass before being accepted or rejected.",
        ]
    )
else:
    lines.extend(
