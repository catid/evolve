...
ROOT="outputs/diagnostics/architecture_route_bias_keyed_residual_predictive_delta_mix_probe"
REPORT_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_mix_probe.md"
REPORT_JSON="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_mix_probe.json"
RUN_SUMMARY_MD="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_mix_probe_run_summary.md"
RUN_SUMMARY_CSV="outputs/reports/architecture_route_bias_keyed_residual_predictive_delta_mix_probe_run_summary.csv"
...
declare -a CONFIGS=(
  ...
  "configs/diagnostic/minigrid_doorkey_sare_phase_memory_route_bias_keyed_residual_predictive_delta_mix_probe.yaml"
)
...
manifest = {
    ...
    "family": "architecture_route_bias_keyed_residual_predictive_delta_mix_probe",
...
for metrics_path in sorted(root.glob("*/metrics.jsonl")):
    ...
    if "sare_phase_memory_route_bias_keyed_residual_predictive_delta_mix" in config_text:
    ...
    rows.append(
        {
            ...
            "memory_mixed_predictive_delta_logits_norm": float(last.get("memory/mixed_predictive_delta_logits_norm", 0.0)),
...
def summarize(label: str) -> dict[str, float]:
    ...
    keys = [
        ...
        "memory_mixed_predictive_delta_logits_norm",
...
variants = {
    ...
    "sare_phase_memory_route_bias_keyed_residual_predictive_delta_mix": summarize("sare_phase_memory_route_bias_keyed_residual_predictive_delta_mix"),
}
report_json.write_text(json.dumps({"rows": rows, "variants": variants}, indent=2, sort_keys=True), encoding="utf-8")

lines = [
    "# Predictive-Delta Mix Keyed-Residual Route-Bias Architecture Probe",
    "",
    "- variants: `sare_phase_memory`, `sare_phase_memory_route_bias`, `sare_phase_memory_route_bias_keyed_residual`, `sare_phase_memory_route_bias_keyed_residual_predictive_delta_mix`",
    ...
    "## Aggregate",
    "",
    "| Variant | Runs | Eval Success | Eval Return | Train Success | Throughput | Route Entropy | Route Bias Norm | Base Logits Norm | Prior Keyed Norm | Predictive Keyed Norm | Final Keyed Norm | Mixed Delta Norm | Total Logits Norm | Predictive Delta Norm |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for label in (
    "sare_phase_memory",
    ...
    lines.append(
        ...
        + f"`{summary.get('memory_mixed_predictive_delta_logits_norm', 0.0):.3f}` | "
...
lines.extend(
    [
        ...
        "## Per-Run",
        "",
        "| Variant | Seed | Eval Success | Eval Return | Train Success | Throughput | Prior Keyed Norm | Predictive Keyed Norm | Mixed Delta Norm | Run Dir |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
)
for row in sorted(rows, key=lambda item: (item["variant"], item["seed"])):
    lines.append(
        ...
        + f"`{row['memory_mixed_predictive_delta_logits_norm']:.3f}` | "
        + f"`{row['run_dir']}` |"
    )
