# Causal Route-Dependence Report

- external evaluation episodes per probe: `64`
- git commit: `21a890285831feb89c6de03ea16619239ff732e3`
- git dirty: `True`

| Lane | Seed | Probe | Detail | Greedy Success | Greedy Return | Route Entropy | Active Compute |
| --- | --- | --- | --- | ---: | ---: | ---: | ---: |
| fresh_final | 53 | baseline | - | 1.0000 | 0.9638 | 1.3787 | 0.5000 |
| fresh_final | 53 | expert_ablation | 0 | 0.0000 | 0.0000 | 1.3790 | 0.5000 |
| fresh_final | 53 | expert_ablation | 1 | 0.0000 | 0.0000 | 1.3788 | 0.5000 |
| fresh_final | 53 | expert_ablation | 2 | 1.0000 | 0.9641 | 1.3787 | 0.5000 |
| fresh_final | 53 | expert_ablation | 3 | 0.0000 | 0.0000 | 1.3793 | 0.5000 |
| fresh_final | 53 | router_override | most_used_pair | 0.0000 | 0.0000 | 0.6931 | 0.5000 |
| fresh_final | 53 | route_randomization | uniform_topk_random | 0.0000 | 0.0000 | 0.6931 | 0.5000 |

## Interpretation

- `fresh_final` seed `53` uses top experts `[3, 1]` for the fixed-router probe. Baseline greedy success is `1.0000`; fixed-router drop is `1.0000`, random-routing drop is `1.0000`, and worst single-expert ablation drop is `1.0000` (expert `0`).
- Learned routing assignments are causally relevant under this bounded probe family: disrupting routing materially degrades greedy DoorKey success.
