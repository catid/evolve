...
def test_por_actor_hidden_film_logs_metrics(tmp_path: Path) -> None:
    ...
    assert "policy/option_hidden_scale_floor_power" in last_scalar
...
def test_por_actor_hidden_gate_bias_logs_metrics(tmp_path: Path) -> None:
    config = load_config("configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_gatebias20.yaml")
    config.system.device = "cpu"
    config.logging.tensorboard = False
    config.env.num_envs = 2
    config.env.num_eval_envs = 1
    ...
    config.ppo.minibatches = 2
    config.evaluation.episodes = 1
    config.logging.output_dir = str(tmp_path / "memory_por_actor_hidden_gate_bias")
    ...
    for line in Path(config.logging.output_dir, "metrics.jsonl").read_text().splitlines():
        ...
        if row.get("type") == "scalar":
            last_scalar = row
    assert "policy/option_hidden_gate_bias" in last_scalar
    assert "policy/option_hidden_gate_bias_scale" in last_scalar
    assert "policy/option_hidden_gate_bias_mean" in last_scalar
    assert "policy/option_hidden_scale_gate_signal_mean" in last_scalar
    assert "policy/option_hidden_shift_gate_signal_mean" in last_scalar


def test_por_actor_hidden_branch_gate_bias_logs_metrics(tmp_path: Path) -> None:
    config = load_config("configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_scalebias10.yaml")
    config.system.device = "cpu"
    config.logging.tensorboard = False
    config.env.num_envs = 2
    config.env.num_eval_envs = 1
    ...
    config.ppo.minibatches = 2
    config.evaluation.episodes = 1
    config.logging.output_dir = str(tmp_path / "memory_por_actor_hidden_branch_gate_bias")
    ...
    for line in Path(config.logging.output_dir, "metrics.jsonl").read_text().splitlines():
        ...
        if row.get("type") == "scalar":
            last_scalar = row
    assert "policy/option_hidden_scale_gate_bias_scale" in last_scalar
    assert "policy/option_hidden_shift_gate_bias_scale" in last_scalar
    assert "policy/option_hidden_scale_gate_bias_mean" in last_scalar
    assert "policy/option_hidden_shift_gate_bias_mean" in last_scalar


def test_por_actor_hidden_adaptive_shift_floor_logs_metrics(tmp_path: Path) -> None:
    config = load_config("configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftfloor18.yaml")
    config.system.device = "cpu"
    config.logging.tensorboard = False
    config.env.num_envs = 2
    config.env.num_eval_envs = 1
    ...
    config.ppo.minibatches = 2
    config.evaluation.episodes = 1
    config.logging.output_dir = str(tmp_path / "memory_por_actor_hidden_adaptive_shift_floor")
    ...
    for line in Path(config.logging.output_dir, "metrics.jsonl").read_text().splitlines():
        ...
        if row.get("type") == "scalar":
            last_scalar = row
    assert "policy/option_hidden_adaptive_shift_floor" in last_scalar
    assert "policy/option_hidden_shift_floor" in last_scalar
    assert "policy/option_hidden_shift_floor_power" in last_scalar
    assert "policy/option_hidden_adaptive_shift_weight_mean" in last_scalar


def test_por_actor_hidden_shift_gate_power_logs_metrics(tmp_path: Path) -> None:
    config = load_config("configs/experiments/minigrid_memory_por_switchy_actor_hidden_partial_shift22_shiftgate150.yaml")
    config.system.device = "cpu"
    config.logging.tensorboard = False
    config.env.num_envs = 2
    config.env.num_eval_envs = 1
    ...
    config.ppo.minibatches = 2
    config.evaluation.episodes = 1
    config.logging.output_dir = str(tmp_path / "memory_por_actor_hidden_shift_gate_power")
