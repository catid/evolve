...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert "memory/weak_base_expert_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/keyed_residual_weak_base_boost" in output.metrics
    assert "memory/weak_base_factor" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()
...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_margin_advantage_expert_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_margin_advantage_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/margin_advantage_factor" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_sign_disagreement_expert_gate_reports_gate_statistics() -> None:
    ...
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_sign_disagreement_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/sign_disagreement_gate_mean" in output.metrics
    assert "memory/combined_weak_sign_disagreement_gate_mean" in output.metrics
    assert "memory/combined_weak_sign_disagreement_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/sign_disagreement_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_sign_disagreement_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_sign_disagreement_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()


def test_sare_phase_memory_route_bias_keyed_residual_weak_base_cosine_disagreement_expert_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_cosine_disagreement_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/base_keyed_alignment_cosine" in output.metrics
    assert "memory/cosine_disagreement_gate_mean" in output.metrics
    assert "memory/cosine_disagreement_gate_max" in output.metrics
    assert "memory/combined_weak_cosine_disagreement_gate_mean" in output.metrics
    assert "memory/combined_weak_cosine_disagreement_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert -1.0 <= float(output.metrics["memory/base_keyed_alignment_cosine"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/cosine_disagreement_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/cosine_disagreement_gate_max"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_cosine_disagreement_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_cosine_disagreement_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()


def test_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_softmax_redistribute_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_expert_softmax_redistribute",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/redistributed_weak_base_gate_mean" in output.metrics
    assert "memory/redistributed_weak_base_gate_std" in output.metrics
    assert "memory/redistributed_weak_base_gate_max" in output.metrics
    assert "memory/weak_base_redistribution_weight_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/weak_base_softmax_temperature" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/redistributed_weak_base_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/weak_base_redistribution_weight_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()
...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_sign_aligned_expert_gate_reports_gate_statistics() -> None:
    ...
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_sign_aligned_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/sign_alignment_gate_mean" in output.metrics
    assert "memory/combined_sign_aligned_weak_base_gate_mean" in output.metrics
    assert "memory/combined_sign_aligned_weak_base_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/sign_alignment_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_sign_aligned_weak_base_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_sign_aligned_weak_base_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()


def test_sare_phase_memory_route_bias_keyed_residual_weak_base_keyed_top2_expert_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_keyed_top2_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/keyed_positive_gate_mean" in output.metrics
    assert "memory/keyed_top2_gate_mean" in output.metrics
    assert "memory/combined_weak_keyed_top2_gate_mean" in output.metrics
    assert "memory/combined_weak_keyed_top2_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/keyed_top2_count" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/keyed_positive_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/keyed_top2_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_keyed_top2_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_keyed_top2_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    assert float(output.metrics["memory/keyed_top2_count"]) == 2.0
    env.close()
...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_keyed_relative_expert_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_keyed_relative_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/keyed_relative_gate_mean" in output.metrics
    assert "memory/keyed_relative_gate_std" in output.metrics
    assert "memory/keyed_relative_gate_max" in output.metrics
    assert "memory/combined_weak_keyed_relative_gate_mean" in output.metrics
    assert "memory/combined_weak_keyed_relative_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/keyed_relative_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/keyed_relative_gate_max"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_keyed_relative_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_keyed_relative_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()
...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_gate_reports_gate_statistics() -> None:
    ...
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/high_base_gate_mean" in output.metrics
    assert "memory/high_base_gate_max" in output.metrics
    assert "memory/combined_weak_high_base_gate_mean" in output.metrics
    assert "memory/combined_weak_high_base_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/high_base_target_norm" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/high_base_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/high_base_gate_max"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_high_base_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_high_base_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()


def test_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_expert_high_base_floor_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/high_base_gate_mean" in output.metrics
    assert "memory/effective_high_base_floor_gate_mean" in output.metrics
    assert "memory/effective_high_base_floor_gate_max" in output.metrics
    assert "memory/combined_weak_high_base_gate_mean" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/low_base_boost_floor" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/high_base_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/effective_high_base_floor_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/effective_high_base_floor_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()


def test_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_low_base_renorm_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_expert_low_base_renorm",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/low_base_renorm_gate_mean" in output.metrics
    assert "memory/low_base_renorm_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/renormed_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/adaptive_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/keyed_residual_norm_preserve_scale_mean" in output.metrics
    assert "memory/low_base_target_norm" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/low_base_renorm_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/low_base_renorm_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    assert float(output.metrics["memory/keyed_residual_norm_preserve_scale_mean"]) > 0.0
    env.close()


def test_sare_phase_memory_route_bias_keyed_residual_weak_base_expert_renorm_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
    ...
    assert "memory/weak_base_expert_gate_std" in output.metrics
    assert "memory/weak_base_expert_gate_max" in output.metrics
    assert "memory/reweighted_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/renormed_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/keyed_residual_norm_preserve_scale_mean" in output.metrics
    assert "memory/keyed_residual_weak_base_boost" in output.metrics
...
def test_sare_phase_memory_route_bias_keyed_residual_weak_base_weak_keyed_expert_gate_reports_gate_statistics() -> None:
    env, obs, done = _obs()
    model = build_model(
        ModelConfig(
            variant="sare_phase_memory_route_bias_keyed_residual_weak_base_weak_keyed_expert_gate",
            expert_count=4,
            top_k=2,
            memory_mix=0.5,
            route_memory_scale=0.5,
    ...
    assert output.next_state["hidden"].shape == (1, 128)
    assert "memory/weak_base_expert_gate_mean" in output.metrics
    assert "memory/weak_keyed_expert_gate_mean" in output.metrics
    assert "memory/combined_weak_expert_gate_mean" in output.metrics
    assert "memory/combined_weak_expert_gate_max" in output.metrics
    assert "memory/gated_keyed_route_bias_logits_norm" in output.metrics
    assert "memory/keyed_residual_scale_mean" in output.metrics
    assert "memory/weak_keyed_factor" in output.metrics
    assert float(output.metrics["memory/route_bias_scale"]) == 0.5
    assert 0.0 <= float(output.metrics["memory/weak_base_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/weak_keyed_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_expert_gate_mean"]) <= 1.0
    assert 0.0 <= float(output.metrics["memory/combined_weak_expert_gate_max"]) <= 1.0
    assert 1.0 <= float(output.metrics["memory/keyed_residual_scale_mean"]) <= 1.5
    env.close()
