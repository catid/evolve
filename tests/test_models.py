...
def test_por_actor_hidden_film_reports_metrics() -> None:
    ...
    model = build_model(
        ModelConfig(
            ...
            policy_option_hidden_scale_floor_power=0.75,
    ...
    assert "policy/option_hidden_scale_floor_power" in output.metrics
    ...
    assert "policy/option_hidden_center_shift" in output.metrics
    assert "policy/option_hidden_center_shift_scale" in output.metrics
    assert "policy/option_hidden_center_shift_mean_abs" in output.metrics
...
def test_por_actor_hidden_gate_bias_reports_metrics() -> None:
    env = gym.make("MiniGrid-MemoryS9-v0")
    obs = _sample_obs(env)
    done = torch.ones(1, dtype=torch.bool)
    model = build_model(
        ModelConfig(
            ...
            policy_option_hidden_gate_bias=True,
            policy_option_hidden_gate_bias_scale=0.1,
        ),
        env.observation_space,
        env.action_space,
    )
    state = model.initial_state(batch_size=1, device=torch.device("cpu"))
    output = model.forward(obs, state=state, done=done)
    assert "policy/option_hidden_gate_bias" in output.metrics
    assert "policy/option_hidden_gate_bias_scale" in output.metrics
    assert "policy/option_hidden_gate_bias_mean" in output.metrics
    assert "policy/option_hidden_scale_gate_signal_mean" in output.metrics
    assert "policy/option_hidden_shift_gate_signal_mean" in output.metrics
    env.close()


def test_por_actor_hidden_branch_gate_bias_reports_metrics() -> None:
    env = gym.make("MiniGrid-MemoryS9-v0")
    obs = _sample_obs(env)
    done = torch.ones(1, dtype=torch.bool)
    model = build_model(
        ModelConfig(
            ...
            policy_option_hidden_shift_weight=0.22,
            policy_option_hidden_gate_bias=True,
            policy_option_hidden_scale_gate_bias_scale=0.1,
            policy_option_hidden_shift_gate_bias_scale=0.05,
        ),
        env.observation_space,
        env.action_space,
    )
    ...
    output = model.forward(obs, state=state, done=done)
    assert "policy/option_hidden_gate_bias" in output.metrics
    assert "policy/option_hidden_scale_gate_bias_scale" in output.metrics
    assert "policy/option_hidden_shift_gate_bias_scale" in output.metrics
    assert "policy/option_hidden_scale_gate_bias_mean" in output.metrics
    assert "policy/option_hidden_shift_gate_bias_mean" in output.metrics
    env.close()
...
def test_por_actor_hidden_shift_gate_power_reports_metrics() -> None:
    env = gym.make("MiniGrid-MemoryS9-v0")
    obs = _sample_obs(env)
    done = torch.ones(1, dtype=torch.bool)
    model = build_model(
        ModelConfig(
            ...
            policy_option_hidden_use_duration_gate=True,
            policy_option_hidden_shift_weight=0.22,
            policy_option_hidden_shift_gate_power=1.25,
        ),
        env.observation_space,
        env.action_space,
    )
    state = model.initial_state(batch_size=1, device=torch.device("cpu"))
    output = model.forward(obs, state=state, done=done)
    assert "policy/option_hidden_scale_gate_power" in output.metrics
    assert "policy/option_hidden_shift_gate_power" in output.metrics
    assert "policy/option_hidden_scale_gate_signal_mean" in output.metrics
    assert "policy/option_hidden_shift_gate_signal_mean" in output.metrics
    env.close()


def test_por_actor_hidden_scale_gate_power_reports_metrics() -> None:
    env = gym.make("MiniGrid-MemoryS9-v0")
    obs = _sample_obs(env)
    done = torch.ones(1, dtype=torch.bool)
    model = build_model(
        ModelConfig(
            ...
            policy_option_hidden_use_duration_gate=True,
            policy_option_hidden_shift_weight=0.22,
            policy_option_hidden_scale_gate_power=1.25,
        ),
        env.observation_space,
        env.action_space,
    )
