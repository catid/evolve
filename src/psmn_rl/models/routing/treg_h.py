...
class TREGHCore(nn.Module):
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        halt_prob = torch.sigmoid(self.halt_head(hop_1))
        ...
        mixed = halt_prob * hop_1 + (1.0 - halt_prob) * hop_2
        ...
        metrics = {
            ...
            "avg_hop_count": float((1.0 + (1.0 - halt_prob)).mean().item()),
            "avg_halting_probability": float(halt_prob.mean().item()),
            "active_compute_proxy": float(1.0 + (1.0 - halt_prob).mean().item()),
        }
        avg_usage = 0.5 * (
        ...
        aux_losses = {"ponder_loss": (1.0 - halt_prob).mean() * self.ponder_cost}
