...
class RoutedExpertRouteBiasedKeyedResidualWeakBaseMarginAdvantageExpertGatedPhaseMemoryCore(RoutedExpertPhaseMemoryCore):
    def __init__(
        self,
        observation_space,
        token_dim: int,
        ...
        keyed_residual_weak_base_boost: float = 0.5,
        weak_base_factor: float = 1.5,
        margin_advantage_factor: float = 1.0,
    ) -> None:
        super().__init__(
            observation_space=observation_space,
            token_dim=token_dim,
        ...
        self.keyed_residual_weak_base_boost = float(keyed_residual_weak_base_boost)
        self.weak_base_factor = float(weak_base_factor)
        self.margin_advantage_factor = float(margin_advantage_factor)
        self.route_bias_proj = nn.Linear(hidden_size, expert_count)
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        base_scale = base_abs.mean(dim=-1, keepdim=True).clamp_min(1e-6)
        weak_base_gate = torch.clamp(1.0 - (base_abs / (base_scale * self.weak_base_factor)), min=0.0, max=1.0)
        margin_advantage_logits = torch.relu(keyed_route_logits - base_route_logits)
        margin_advantage_gate = torch.clamp(
            margin_advantage_logits / (base_scale * self.margin_advantage_factor),
            min=0.0,
            max=1.0,
        )
        combined_gate = weak_base_gate * margin_advantage_gate
        ...
        metrics = {
            ...
            **memory_metrics,
            ...
            "memory/weak_base_expert_gate_std": float(weak_base_gate.std(unbiased=False).item()),
            "memory/weak_base_expert_gate_max": float(weak_base_gate.max().item()),
            "memory/margin_advantage_gate_mean": float(margin_advantage_gate.mean().item()),
            "memory/margin_advantage_gate_max": float(margin_advantage_gate.max().item()),
            "memory/combined_weak_margin_advantage_gate_mean": float(combined_gate.mean().item()),
            "memory/combined_weak_margin_advantage_gate_max": float(combined_gate.max().item()),
            "memory/keyed_residual_scale_mean": float(keyed_residual_scale.mean().item()),
            "memory/keyed_residual_weak_base_boost": self.keyed_residual_weak_base_boost,
            "memory/weak_base_factor": self.weak_base_factor,
            "memory/margin_advantage_factor": self.margin_advantage_factor,
        }
        return CoreOutput(
            pooled=pooled,
            tokens=tokens,
...
class RoutedExpertRouteBiasedKeyedResidualWeakBaseMarginBonusExpertGatedPhaseMemoryCore(RoutedExpertPhaseMemoryCore):
    def __init__(
        self,
        observation_space,
        token_dim: int,
        ...
        self.weak_base_factor = float(weak_base_factor)
        self.margin_advantage_factor = float(margin_advantage_factor)
        self.margin_bonus_factor = float(margin_bonus_factor)
        self.route_bias_proj = nn.Linear(hidden_size, expert_count)
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        combined_gate = weak_base_gate * margin_advantage_gate
        margin_bonus_logits = margin_advantage_logits * weak_base_gate * self.margin_bonus_factor
        route_bias_logits = base_route_logits + keyed_route_logits + margin_bonus_logits
        route_bias = torch.tanh(route_bias_logits) * self.route_memory_scale
        route_probs, topk_values, topk_idx = self.route(tokens, route_bias=route_bias)
        mixed = self.apply_experts(tokens, topk_values, topk_idx)
        tokens = self.output_norm(tokens + mixed)
        ...
        metrics = {
            ...
            **memory_metrics,
            ...
            "memory/base_route_bias_logits_norm": float(base_route_logits.norm(dim=-1).mean().item()),
            "memory/keyed_route_bias_logits_norm": float(keyed_route_logits.norm(dim=-1).mean().item()),
            "memory/gated_keyed_route_bias_logits_norm": float(keyed_route_logits.norm(dim=-1).mean().item()),
            "memory/margin_advantage_logits_norm": float(margin_advantage_logits.norm(dim=-1).mean().item()),
            "memory/margin_bonus_logits_norm": float(margin_bonus_logits.norm(dim=-1).mean().item()),
            "memory/route_bias_logits_norm": float(route_bias_logits.norm(dim=-1).mean().item()),
            "memory/route_memory_query_norm": float(route_memory_query.norm(dim=-1).mean().item()),
            "memory/weak_base_expert_gate_mean": float(weak_base_gate.mean().item()),
            "memory/weak_base_expert_gate_std": float(weak_base_gate.std(unbiased=False).item()),
            ...
            "memory/combined_weak_margin_advantage_gate_mean": float(combined_gate.mean().item()),
            "memory/combined_weak_margin_advantage_gate_max": float(combined_gate.max().item()),
            "memory/margin_advantage_density": float((margin_advantage_logits > 0).to(dtype=route_bias.dtype).mean().item()),
            "memory/weak_base_factor": self.weak_base_factor,
            "memory/margin_advantage_factor": self.margin_advantage_factor,
            "memory/margin_bonus_factor": self.margin_bonus_factor,
        }
        return CoreOutput(
            pooled=pooled,
            tokens=tokens,
...
class RoutedExpertRouteBiasedKeyedResidualWeakBaseSignDisagreementExpertGatedPhaseMemoryCore(RoutedExpertPhaseMemoryCore):
    def __init__(
        self,
        observation_space,
        token_dim: int,
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        base_scale = base_abs.mean(dim=-1, keepdim=True).clamp_min(1e-6)
        weak_base_gate = torch.clamp(1.0 - (base_abs / (base_scale * self.weak_base_factor)), min=0.0, max=1.0)
        sign_disagreement_gate = (base_route_logits * keyed_route_logits < 0).to(dtype=base_route_logits.dtype)
        combined_gate = weak_base_gate * sign_disagreement_gate
        keyed_residual_scale = 1.0 + combined_gate * self.keyed_residual_weak_base_boost
        gated_keyed_route_logits = keyed_route_logits * keyed_residual_scale
        route_bias_logits = base_route_logits + gated_keyed_route_logits
        route_bias = torch.tanh(route_bias_logits) * self.route_memory_scale
        ...
        metrics = {
            ...
            **memory_metrics,
            ...
            "memory/weak_base_expert_gate_std": float(weak_base_gate.std(unbiased=False).item()),
            "memory/weak_base_expert_gate_max": float(weak_base_gate.max().item()),
            "memory/sign_disagreement_gate_mean": float(sign_disagreement_gate.mean().item()),
            "memory/combined_weak_sign_disagreement_gate_mean": float(combined_gate.mean().item()),
            "memory/combined_weak_sign_disagreement_gate_max": float(combined_gate.max().item()),
            "memory/keyed_residual_scale_mean": float(keyed_residual_scale.mean().item()),
            "memory/keyed_residual_weak_base_boost": self.keyed_residual_weak_base_boost,
            "memory/weak_base_factor": self.weak_base_factor,
        }
...
class RoutedExpertRouteBiasedKeyedResidualWeakBaseExpertConcentrationBlendedPhaseMemoryCore(RoutedExpertPhaseMemoryCore):
    def __init__(
        self,
        observation_space,
        token_dim: int,
        ...
        keyed_residual_weak_base_boost: float = 0.5,
        weak_base_factor: float = 1.5,
        weak_base_concentration_target: float = 0.25,
    ) -> None:
        super().__init__(
            observation_space=observation_space,
            token_dim=token_dim,
        ...
        self.keyed_residual_weak_base_boost = float(keyed_residual_weak_base_boost)
        self.weak_base_factor = float(weak_base_factor)
        self.weak_base_concentration_target = float(weak_base_concentration_target)
        self.route_bias_proj = nn.Linear(hidden_size, expert_count)
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        base_scale = base_abs.mean(dim=-1, keepdim=True).clamp_min(1e-6)
        weak_base_gate = torch.clamp(1.0 - (base_abs / (base_scale * self.weak_base_factor)), min=0.0, max=1.0)
        weak_base_gate_std = weak_base_gate.std(dim=-1, keepdim=True, unbiased=False)
        concentration_gate = torch.clamp(
            weak_base_gate_std / self.weak_base_concentration_target,
            min=0.0,
            max=1.0,
        )
        keyed_residual_scale = 1.0 + weak_base_gate * self.keyed_residual_weak_base_boost * concentration_gate
        gated_keyed_route_logits = keyed_route_logits * keyed_residual_scale
        route_bias_logits = base_route_logits + gated_keyed_route_logits
        route_bias = torch.tanh(route_bias_logits) * self.route_memory_scale
        route_probs, topk_values, topk_idx = self.route(tokens, route_bias=route_bias)
        ...
        metrics = {
            ...
            **memory_metrics,
            ...
            "memory/weak_base_expert_gate_std": float(weak_base_gate.std(unbiased=False).item()),
            "memory/weak_base_expert_gate_max": float(weak_base_gate.max().item()),
            "memory/weak_base_concentration_gate_mean": float(concentration_gate.mean().item()),
            "memory/weak_base_concentration_gate_max": float(concentration_gate.max().item()),
            "memory/keyed_residual_scale_mean": float(keyed_residual_scale.mean().item()),
            "memory/keyed_residual_weak_base_boost": self.keyed_residual_weak_base_boost,
            "memory/weak_base_factor": self.weak_base_factor,
            "memory/weak_base_concentration_target": self.weak_base_concentration_target,
        }
        return CoreOutput(
            pooled=pooled,
            tokens=tokens,
...
class RoutedExpertRouteBiasedKeyedResidualWeakBaseCosineDisagreementExpertGatedPhaseMemoryCore(RoutedExpertPhaseMemoryCore):
    def __init__(
        self,
        observation_space,
        token_dim: int,
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        base_norm = base_route_logits.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        keyed_norm = keyed_route_logits.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        alignment_cosine = (base_route_logits * keyed_route_logits).sum(dim=-1, keepdim=True) / (base_norm * keyed_norm)
        disagreement_gate = torch.clamp((1.0 - alignment_cosine) * 0.5, min=0.0, max=1.0)
        combined_gate = weak_base_gate * disagreement_gate
        keyed_residual_scale = 1.0 + combined_gate * self.keyed_residual_weak_base_boost
        gated_keyed_route_logits = keyed_route_logits * keyed_residual_scale
        route_bias_logits = base_route_logits + gated_keyed_route_logits
        route_bias = torch.tanh(route_bias_logits) * self.route_memory_scale
        ...
        metrics = {
            ...
            **memory_metrics,
            ...
            "memory/weak_base_expert_gate_std": float(weak_base_gate.std(unbiased=False).item()),
            "memory/weak_base_expert_gate_max": float(weak_base_gate.max().item()),
            "memory/base_keyed_alignment_cosine": float(alignment_cosine.mean().item()),
            "memory/cosine_disagreement_gate_mean": float(disagreement_gate.mean().item()),
            "memory/cosine_disagreement_gate_max": float(disagreement_gate.max().item()),
            "memory/combined_weak_cosine_disagreement_gate_mean": float(combined_gate.mean().item()),
            "memory/combined_weak_cosine_disagreement_gate_max": float(combined_gate.max().item()),
            "memory/keyed_residual_scale_mean": float(keyed_residual_scale.mean().item()),
            "memory/keyed_residual_weak_base_boost": self.keyed_residual_weak_base_boost,
            "memory/weak_base_factor": self.weak_base_factor,
        }
...
class RoutedExpertRouteBiasedKeyedResidualWeakBaseExpertSoftmaxRedistributedPhaseMemoryCore(RoutedExpertPhaseMemoryCore):
    def __init__(
        self,
        observation_space,
        token_dim: int,
        ...
        keyed_residual_weak_base_boost: float = 0.5,
        weak_base_factor: float = 1.5,
        weak_base_softmax_temperature: float = 0.35,
    ) -> None:
        super().__init__(
            observation_space=observation_space,
            token_dim=token_dim,
        ...
        self.keyed_residual_weak_base_boost = float(keyed_residual_weak_base_boost)
        self.weak_base_factor = float(weak_base_factor)
        self.weak_base_softmax_temperature = float(weak_base_softmax_temperature)
        self.route_bias_proj = nn.Linear(hidden_size, expert_count)
    ...
    def forward(self, obs: dict[str, torch.Tensor], state: dict[str, torch.Tensor], done: torch.Tensor | None) -> CoreOutput:
        ...
        base_scale = base_abs.mean(dim=-1, keepdim=True).clamp_min(1e-6)
        weak_base_gate = torch.clamp(1.0 - (base_abs / (base_scale * self.weak_base_factor)), min=0.0, max=1.0)
        weak_base_gate_mean = weak_base_gate.mean(dim=-1, keepdim=True)
        redistribution_weights = torch.softmax(
            weak_base_gate / self.weak_base_softmax_temperature,
            dim=-1,
        )
        redistributed_gate = weak_base_gate_mean * redistribution_weights * self.expert_count
        keyed_residual_scale = 1.0 + redistributed_gate * self.keyed_residual_weak_base_boost
        gated_keyed_route_logits = keyed_route_logits * keyed_residual_scale
        route_bias_logits = base_route_logits + gated_keyed_route_logits
        route_bias = torch.tanh(route_bias_logits) * self.route_memory_scale
        route_probs, topk_values, topk_idx = self.route(tokens, route_bias=route_bias)
        ...
        metrics = {
            ...
            **memory_metrics,
            ...
            "memory/weak_base_expert_gate_std": float(weak_base_gate.std(unbiased=False).item()),
            "memory/weak_base_expert_gate_max": float(weak_base_gate.max().item()),
            "memory/redistributed_weak_base_gate_mean": float(redistributed_gate.mean().item()),
            "memory/redistributed_weak_base_gate_std": float(redistributed_gate.std(unbiased=False).item()),
            "memory/redistributed_weak_base_gate_max": float(redistributed_gate.max().item()),
            "memory/weak_base_redistribution_weight_max": float(redistribution_weights.max().item()),
            "memory/keyed_residual_scale_mean": float(keyed_residual_scale.mean().item()),
            "memory/keyed_residual_weak_base_boost": self.keyed_residual_weak_base_boost,
            "memory/weak_base_factor": self.weak_base_factor,
            "memory/weak_base_softmax_temperature": self.weak_base_softmax_temperature,
        }
        return CoreOutput(
            pooled=pooled,
            tokens=tokens,
