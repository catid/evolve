...
BASELINE_STAGE_LABELS: tuple[str, ...] = (
...
def _candidate_row(record: RunRecord) -> dict[str, Any]:
    ...
    return {
        ...
        "final_round_added_steps": _float(final_round.get("added_steps")),
        "final_round_fine_tune_steps": _float(final_round.get("fine_tune/steps")),
    }


def _load_train_run_summary(run_dir: Path) -> dict[str, Any]:
    ...
    if metrics_path.exists():
        ...
        if final_eval is not None:
            return {
                "eval_greedy_success": _float(final_eval.get("eval_success_rate")),
                "eval_return": _float(final_eval.get("eval_return")),
                "global_step": _float(final_eval.get("global_step")),
            }
    ...
    if summary_md.exists():
        ...
        return {
            "eval_greedy_success": _float(payload.get("eval_success_rate")),
            "eval_return": _float(payload.get("eval_return")),
            "global_step": _float(payload.get("global_step")),
        }
    return {}


def _block_specs(campaign: dict[str, Any], key: str) -> list[dict[str, Any]]:
    if key in {"hard_family_dev", "hard_family_holdout"}:
        definition_path = Path(campaign["reports"]["definition_json"])
        if definition_path.exists():
            payload = _read_json(definition_path)
            dynamic_key = f"{key}_blocks"
            if dynamic_key in payload:
                return [dict(item) for item in payload[dynamic_key]]
...
def _lane_seed_pairs(campaign: dict[str, Any], key: str) -> list[tuple[str, int]]:
    specs = _block_specs(campaign, key)
    return [(str(block["lane"]), int(seed)) for block in specs for seed in block.get("seeds", [])]


def _baseline_stage_root(campaign: dict[str, Any], lane: str) -> Path:
    return Path(campaign["stage_roots"]["stage0_blocks"]) / lane


def _new_block_rows(campaign: dict[str, Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for block in campaign["seed_groups"]["new_block_pool"]["blocks"]:
        lane = str(block["lane"])
        root = _baseline_stage_root(campaign, lane)
        for seed in block["seeds"]:
            seed_root = root / f"seed_{seed}"
            recovered_dir = seed_root / "token_dense_ent1e3"
            baseline_sare_dir = seed_root / "sare_ent1e3"
            if recovered_dir.exists():
                recovered_summary = _load_train_run_summary(recovered_dir)
                rows.append(
                    {
                        ...
                        "run_dir": str(recovered_dir),
                        "final_greedy_success": _float(recovered_summary.get("eval_greedy_success")),
                    }
                )
            if baseline_sare_dir.exists():
                sare_summary = _load_train_run_summary(baseline_sare_dir)
                rows.append(
                    {
                        "candidate": str(campaign["current_candidate_name"]),
                        "lane": lane,
                        "seed": int(seed),
                        "label": "baseline_sare",
                        "run_dir": str(baseline_sare_dir),
                        "final_greedy_success": _float(sare_summary.get("eval_greedy_success")),
                    }
                )
            for label in CURRENT_CONTROLS:
                run_dir = seed_root / label
...
def _new_block_summary(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for lane in lanes:
        ...
        stats = {
            ...
            for label in BASELINE_STAGE_LABELS
        }
        summaries.append(
            {
                "lane": lane,
                ...
                "recovered_mean": stats["recovered_token_dense"]["mean"],
                "baseline_sare_mean": stats["baseline_sare"]["mean"],
...
def _definition_payload(campaign: dict[str, Any]) -> dict[str, Any]:
    new_rows = _new_block_rows(campaign)
    new_block_summary = _new_block_summary(new_rows)
    selected_new = [
        row
        for row in sorted(new_block_summary, key=lambda item: (item["candidate_minus_token"], item["candidate_mean"]))
        if row["token_mean"] >= _float(campaign["selection"]["hard_family_min_token_mean"])
        and row["candidate_minus_token"] <= -_float(campaign["selection"]["hard_family_min_token_gap"])
    ]
    dev_new_count = _int(campaign["selection"]["dev_new_block_count"])
    holdout_new_count = _int(campaign["selection"]["holdout_new_block_count"])
    selected_dev_blocks = [dict(item) for item in campaign["seed_groups"]["known_hard_dev"]["blocks"]]
    selected_holdout_blocks = [dict(item) for item in campaign["seed_groups"]["existing_hard_holdout"]["blocks"]]
    if len(selected_new) >= dev_new_count:
        for row in selected_new[:dev_new_count]:
            block = next(item for item in campaign["seed_groups"]["new_block_pool"]["blocks"] if str(item["lane"]) == str(row["lane"]))
            selected_dev_blocks.append(dict(block))
    if len(selected_new) >= dev_new_count + holdout_new_count:
        for row in selected_new[dev_new_count : dev_new_count + holdout_new_count]:
            block = next(item for item in campaign["seed_groups"]["new_block_pool"]["blocks"] if str(item["lane"]) == str(row["lane"]))
            selected_holdout_blocks.append(dict(block))
    definition_ready = len(selected_dev_blocks) >= 3 and len(selected_holdout_blocks) >= 2
    return {
        "current_candidate_name": str(campaign["current_candidate_name"]),
        "selection_thresholds": {
            "hard_family_min_token_gap": _float(campaign["selection"]["hard_family_min_token_gap"]),
            "hard_family_min_token_mean": _float(campaign["selection"]["hard_family_min_token_mean"]),
        },
        "new_block_rows": new_rows,
        "new_block_summary": new_block_summary,
        "qualified_new_blocks": selected_new,
        "hard_family_dev_blocks": selected_dev_blocks,
        "hard_family_holdout_blocks": selected_holdout_blocks,
...
def _render_definition(campaign: dict[str, Any], output: Path, json_output: Path | None = None) -> None:
    payload = _definition_payload(campaign)
    selected_dev_blocks = payload["hard_family_dev_blocks"]
    selected_holdout_blocks = payload["hard_family_holdout_blocks"]
    current_rows = _load_current_rows(
        campaign,
        tuple(str(block["lane"]) for block in selected_dev_blocks + selected_holdout_blocks),
    )
    lines = [
        ...
        f"- definition ready: `{payload['definition_ready']}`",
        "",
        "## Family Definition",
        "",
        ...
        f"- qualification threshold for new blocks: matched token_dense mean at least `{payload['selection_thresholds']['hard_family_min_token_mean']:.2f}` and candidate-minus-token_dense at most `-{payload['selection_thresholds']['hard_family_min_token_gap']:.2f}`.",
        f"- selected development blocks: `{selected_dev_blocks}`",
        f"- selected holdout blocks: `{selected_holdout_blocks}`",
        "",
    ...
    qualified_lanes = {str(row["lane"]) for row in payload["qualified_new_blocks"]}
    for row in payload["new_block_summary"]:
        lines.append(
            "| "
            + " | ".join(
                [
                    ...
                    _format_float(row["recovered_mean"]),
                    _format_float(row["baseline_sare_mean"]),
                    _format_float(row["candidate_minus_token"]),
                    str(int(row["candidate_failures"])),
                    "`yes`" if str(row["lane"]) in qualified_lanes else "`no`",
                ]
            )
            + " |"
        )
...
def _render_registration(campaign: dict[str, Any], output: Path) -> None:
    ...
    lines = [
        ...
        "## Splits",
        "",
        f"- hard-family development blocks: `{payload['hard_family_dev_blocks']}`",
        f"- hard-family holdout blocks: `{payload['hard_family_holdout_blocks']}`",
        f"- anti-regression healthy blocks: `{_block_specs(campaign, 'healthy_blocks')}`",
        "",
        "## Mechanism Directions",
        "",
        f"- total directions: `{len({str(meta['intervention_family']) for meta in campaign['candidates'].values()})}`",
        f"- total screened variants: `{len(campaign['candidates'])}`",
        "",
        "## Stage Gates",
        "",
        ...
        f"- Stage 1 screening: `{campaign['reports']['stage2_report']}`",
        f"- Stage 2 fairness: `{campaign['reports']['stage3_report']}`",
        f"- Stage 3 holdout: `{campaign['reports']['stage4_report']}`",
        f"- Stage 4 anti-regression: `{campaign['reports']['stage5_report']}`",
        f"- Stage 5 route validation: `{campaign['reports']['stage6_report']}`",
        f"- Stage 6 stability: `{campaign['reports']['stage7_report']}`",
...
def _render_baseline_sync(campaign: dict[str, Any], output: Path, csv_output: Path | None = None, json_output: Path | None = None) -> None:
    ...
    current_rows = _load_current_rows(campaign, dev_lanes + holdout_lanes + _block_lanes(campaign, "healthy_blocks"))
    ...
    for row in sorted(
        [row for row in current_rows if str(row["label"]) in CURRENT_CONTROLS],
    ...
    if json_output is not None:
        _write_json(
            json_output,
            {
                "dev_family": {"sare_mean": dev_family["mean"], "token_mean": dev_token["mean"]},
                "holdout_family": {"sare_mean": holdout_family["mean"], "token_mean": holdout_token["mean"]},
...
def _render_stage2(campaign: dict[str, Any], output: Path, csv_output: Path, json_output: Path) -> None:
    ...
    for candidate, rows in sorted(by_candidate.items()):
        ...
        summary = {
            ...
            "mean_added_steps": _stats([_float(row["final_round_added_steps"]) for row in rows])["mean"],
    ...
    advancing = [
        ...
        for row in sorted(
            ...
            reverse=True,
        )[: _int(campaign["selection"].get("stage2_advance_limit"), 4)]
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    _format_float(row["mean_aggregate_steps"]),
                    _format_float(row["mean_fine_tune_steps"]),
...
def _render_stage3(campaign: dict[str, Any], stage2_payload: dict[str, Any], output: Path, csv_output: Path, json_output: Path) -> None:
    ...
    survivors = [
        ...
        for row in sorted(
            ...
            reverse=True,
        )[: _int(campaign["selection"].get("stage3_advance_limit"), 2)]
    ]
    best_candidate = None
    selected_dev_case = None
    if survivors:
        best_summary = max([row for row in candidate_summaries if str(row["candidate"]) in survivors], key=lambda item: (item["candidate_mean"], item["candidate_minus_token"]))
        best_candidate = str(best_summary["candidate"])
        selected_dev_case = _select_best_case(detail_rows, best_candidate)
    lines = [
        "# Hard-Family Saturation Stage 2 Fairness",
        "",
    ...
    lines.extend(
        [
            ...
            "## Candidate Summary",
            "",
            "| Candidate | Dev SARE | Dev token_dense | Dev single_expert | Dev SARE-token | Dev SARE-single | Complete-Seed Failures | Stage 2 |",
            "| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
        ]
    )
    for row in sorted(candidate_summaries, key=lambda item: item["candidate_mean"], reverse=True):
    ...
    lines.extend(
        [
            ...
            "- Stage 2 is the narrowed fairness pass: only candidates that stop clearly trailing matched token_dense on the hard-family dev split and stay competitive with matched single_expert are allowed past this point.",
        ]
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _render_stage4(campaign: dict[str, Any], stage3_payload: dict[str, Any], output: Path, csv_output: Path, json_output: Path) -> None:
    ...
    if not best_candidate:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text("# Hard-Family Saturation Stage 3 Holdout\n\n- skipped: `no candidate survived narrowed fairness`\n", encoding="utf-8")
    ...
    lines = [
        "# Hard-Family Saturation Stage 3 Holdout",
        "",
    ...
    lines.extend(
        [
            ...
            f"- stage-3 status: `{'pass' if stage4_pass else 'stop'}`",
...
def _render_stage5(campaign: dict[str, Any], stage3_payload: dict[str, Any], stage4_payload: dict[str, Any], output: Path, csv_output: Path, json_output: Path) -> None:
    ...
    if not best_candidate or not stage4_payload.get("stage4_pass"):
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text("# Hard-Family Saturation Stage 4 Anti-Regression\n\n- skipped: `candidate did not survive hard-family holdout`\n", encoding="utf-8")
        _write_csv(csv_output, [])
        _write_json(
            json_output,
            {
    ...
    lines.extend(
        [
            ...
            f"- stage-4 status: `{'pass' if stage5_pass else 'stop'}`",
        ]
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _render_stage6(campaign: dict[str, Any], stage3_payload: dict[str, Any], stage4_payload: dict[str, Any], stage5_payload: dict[str, Any], route_rows: list[dict[str, Any]], output: Path, csv_output: Path, json_output: Path) -> None:
    ...
    if not best_candidate or not stage5_payload.get("stage5_pass") or not selected_dev or not selected_holdout or not selected_healthy:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text("# Hard-Family Saturation Stage 5 Route Validation\n\n- skipped: `candidate did not survive anti-regression`\n", encoding="utf-8")
        _write_csv(csv_output, [])
        _write_json(
            json_output,
            {
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            f"- stage-5 status: `{'pass' if stage6_pass else 'stop'}`",
...
def _render_stage7(campaign: dict[str, Any], stage3_payload: dict[str, Any], stage4_payload: dict[str, Any], stage5_payload: dict[str, Any], stage6_payload: dict[str, Any], device: str, episodes: int, output: Path, csv_output: Path, json_output: Path) -> None:
    ...
    if not best_candidate or not stage6_payload.get("stage6_pass") or not selected_dev or not selected_holdout or not selected_healthy:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text("# Hard-Family Saturation Stage 6 Stability\n\n- skipped: `candidate did not survive route validation`\n", encoding="utf-8")
        _write_csv(csv_output, [])
        _write_json(
            json_output,
            {
    ...
    lines = [
        "# Hard-Family Saturation Stage 6 Stability",
        "",
        f"- best candidate: `{best_candidate}`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            f"- stage-6 status: `{'pass' if stage7_pass else 'stop'}`",
        ]
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _render_successor_pack(
    ...
    retry_lane_seeds = _lane_seed_pairs(campaign, "existing_hard_holdout")
    ...
    combined_lines = [
        "# Hard-Family Saturation Candidate Frozen-Comparable Combined Report",
        "",
    ...
    retry_lines = [
        "# Hard-Family Saturation Candidate Holdout / Retry Report",
        "",
    ...
    summary_lines = [
        "# Hard-Family Saturation Candidate Summary",
        "",
        f"- candidate: `{best_candidate}`",
    ...
    successor_lines = [
        "# Hard-Family Saturation Successor Candidate Pack",
        "",
        f"- candidate: `{best_candidate}`",
...
def _write_stop_stage8(reports: dict[str, Any], reason: str, stage: str) -> None:
    ...
    successor_md.write_text(f"# Hard-Family Saturation Successor Candidate Pack\n\n- status: `not generated`\n- reason: `{reason}`\n", encoding="utf-8")
    ...
    gate_md.write_text(f"# Hard-Family Saturation Gate Report\n\n- status: `not run`\n- reason: `{reason}`\n", encoding="utf-8")
...
def _render_decision_memo(stage2_payload: dict[str, Any], stage3_payload: dict[str, Any] | None, stage4_payload: dict[str, Any] | None, stage5_payload: dict[str, Any] | None, stage6_payload: dict[str, Any] | None, stage7_payload: dict[str, Any] | None, gate_payload: dict[str, Any] | None, output: Path) -> None:
    ...
    if not stage2_payload.get("advancing_candidates"):
        ...
        if gate_verdict == "PASS: thaw consideration allowed" and hard_family_fixed and single_ok:
            ...
            stop_stage = "Stage 7"
    lines = [
        "# Hard-Family Saturation Decision Memo",
        "",
        ...
        "## Stage Summary",
        "",
        f"- Stage 1 advancing candidates: `{stage2_payload.get('advancing_candidates', [])}`",
        f"- Stage 2 best candidate: `{best_candidate}`",
        f"- Stage 3 holdout pass: `{None if stage4_payload is None else stage4_payload.get('stage4_pass')}`",
        f"- Stage 4 anti-regression pass: `{None if stage5_payload is None else stage5_payload.get('stage5_pass')}`",
        f"- Stage 5 route pass: `{None if stage6_payload is None else stage6_payload.get('stage6_pass')}`",
        f"- Stage 6 stability pass: `{None if stage7_payload is None else stage7_payload.get('stage7_pass')}`",
        "",
        "## Final Result",
        "",
    ]
    if final_status == "qualified for canonization within DoorKey":
        lines.append("- The saturation-scale hard-family program fixes the hard-family blocker on both development and holdout splits, preserves the broader DoorKey picture, stays routed and stable, and clears the pack-based gate strongly enough to recommend canonization within DoorKey only.")
        ...
        lines.append("- The saturation-scale hard-family program did not remove the remaining hard-family blocker strongly enough to replace the current thaw-qualified candidate. The accepted state remains thaw-qualified but not canonical.")
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Saturation-scale hard-family program for DoorKey.")
    ...
    definition.add_argument("--json-output", default=None)
    ...
    baseline.add_argument("--csv", default=None)
    baseline.add_argument("--json", default=None)
...
def main() -> None:
    ...
    if args.command in {"definition", "registration", "baseline-sync", "shortlist", "stage2-report"}:
        ...
        if args.command == "definition":
            json_output = None if args.json_output in (None, "") else Path(args.json_output)
            _render_definition(campaign, Path(args.output), json_output)
            return
        if args.command == "registration":
            _render_registration(campaign, Path(args.output))
            return
        if args.command == "baseline-sync":
            csv_output = None if args.csv in (None, "") else Path(args.csv)
            json_output = None if args.json in (None, "") else Path(args.json)
            _render_baseline_sync(campaign, Path(args.output), csv_output, json_output)
            return
    ...
    if args.command == "successor-pack":
        ...
        best_candidate = stage3_payload.get("best_candidate")
        if not best_candidate:
            _write_stop_stage8(campaign["reports"], "no candidate survived narrowed fairness", "Stage 2")
            return
        if not stage4_payload.get("stage4_pass"):
            _write_stop_stage8(campaign["reports"], "candidate did not survive hard-family holdout evaluation", "Stage 3")
            return
        if not stage5_payload.get("stage5_pass"):
            _write_stop_stage8(campaign["reports"], "candidate did not survive healthy-block anti-regression", "Stage 4")
            return
        if not stage6_payload.get("stage6_pass"):
            _write_stop_stage8(campaign["reports"], "candidate did not survive route validation", "Stage 5")
            return
        if not stage7_payload.get("stage7_pass"):
            _write_stop_stage8(campaign["reports"], "candidate did not survive stability validation", "Stage 6")
            return
