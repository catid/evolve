...
from psmn_rl.analysis.lss_portfolio_campaign import _decision_strings
...
def _prior_arch_substantive_runs(campaign: dict[str, Any]) -> tuple[int, int]:
    ...
    explore = _optional_json(campaign["analysis"].get("prior_architecture_stage1_explore_json"))
    exploit_rows = list(exploit.get("candidate_summaries", []))
    explore_rows = list(explore.get("candidate_summaries", []))
    seed_count = 1
    for row in [*exploit_rows, *explore_rows]:
        seed_breakout = list(row.get("seed_breakout", []))
        if seed_breakout:
            seed_count = max(seed_count, len(seed_breakout))
    prior = (len(exploit_rows) + len(explore_rows)) * seed_count
    target = max(prior * 2, 60)
    return prior, target
...
def _write_state_reconciliation(campaign: dict[str, Any]) -> None:
    ...
    lines = [
        "# Escape Distill State Reconciliation",
        "",
        f"- program label: `{_analysis_label(campaign, 'Deadlock Escape and Practicalization Program')}`",
        ...
        "",
        f"- prior architecture specialist substantive runs: `{prior_runs}`",
...
def _write_baseline_sync(campaign: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "- The baseline sync stays close enough to the accepted state to proceed into subgroup detection and source-quality probing.",
        ]
    )
...
def render_source_quality(campaign: dict[str, Any], output: Path, json_output: Path | None, csv_output: Path | None, *, device: str) -> None:
    ...
    lines = [
        "# Escape Distill Stage A2 Source Quality",
        "",
...
def run_rescue_stage(campaign: dict[str, Any], *, stage: str, device: str) -> None:
    ...
    _write_json(
        ...
        {
            "program_label": _analysis_label(campaign, "Deadlock-Escape Program"),
            "stage": stage,
...
def render_rescue_screening(campaign: dict[str, Any], output: Path, csv_output: Path | None, json_output: Path | None) -> None:
    ...
    lines = [
        "# Escape Stage B1 Rescue Screening",
        "",
...
def render_rescue_rerun_holdout(campaign: dict[str, Any], output: Path, csv_output: Path | None, json_output: Path | None) -> None:
    ...
    lines = [
        "# Escape Stage B2 Rescue Rerun and Holdout",
        "",
...
def render_rescue_antiregression_route(campaign: dict[str, Any], output: Path, csv_output: Path | None, json_output: Path | None) -> None:
    ...
    lines = [
        "# Escape Stage B3 Anti-Regression and Route Validation",
        "",
...
def render_subgroup_detector(campaign: dict[str, Any], output: Path, json_output: Path | None, *, device: str) -> None:
    ...
    lines = [
        "# Escape Program Subgroup Detector",
        "",
...
def render_registration(campaign: dict[str, Any], output: Path, json_output: Path | None) -> None:
    ...
    source_quality_models = _source_model_specs(campaign)
    source_quality_probe_count = len(source_quality_models) * 4 + sum(1 for spec in source_quality_models if bool(spec.get("routed")))
    rescue_run_count = sum(len(items) for items in rescue_families.values())
    practicalization_min_runs = 24
    architecture_reports = [str(item) for item in campaign["analysis"].get("source_quality_architecture_reports", [])]
    lines = [
        "# Escape Distill Registration",
        "",
        f"- prior architecture specialist substantive runs: `{prior_runs}`",
        f"- target substantive run budget absent stop conditions: `{target_runs}`",
        f"- planned bounded source-quality probe fits: `{source_quality_probe_count}`",
        ...
        f"- architecture-adjacent feasibility cap if justified: `<= 8`",
        f"- architecture reference reports: `{architecture_reports}`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Pruning Rules",
            "",
            "- Every rescue family gets a 3-4 variant calibration mini-sweep before any prune call.",
            ...
            "- Architecture-adjacent work stays off unless rescue is real, practicalization fails, and the broader oracle ceiling story changes enough to justify a follow-on branch.",
            "- Final allowed outcomes remain: active benchmark confirmed and bounded rescue frontier clarified, challenger replaces the active benchmark, active benchmark remains and the frontier narrows further, architecture-adjacent branch justified for the next phase, or benchmark state itself needs narrowing.",
        ]
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    if json_output is not None:
        _write_json(
            json_output,
            {
                "prior_arch_substantive_runs": prior_runs,
                "target_substantive_runs": target_runs,
                "source_quality_probe_count": source_quality_probe_count,
...
def render_practicalization_screening(campaign: dict[str, Any], output: Path, json_output: Path | None) -> None:
    ...
    if not bool(rescue_stage3.get("bounded_rescue_justified")):
        _render_skip_report(
            "Escape Stage C1 Practicalization Screening",
            output,
            json_output,
...
def render_practicalization_verification(campaign: dict[str, Any], output: Path, json_output: Path | None) -> None:
    ...
    if not bool(rescue_stage3.get("bounded_rescue_justified")):
        _render_skip_report(
            "Escape Stage C2 Practicalization Verification, Fairness, and Holdout",
            output,
            json_output,
...
def render_practicalization_route_stability(campaign: dict[str, Any], output: Path, json_output: Path | None) -> None:
    ...
    if not bool(rescue_stage3.get("bounded_rescue_justified")):
        _render_skip_report(
            "Escape Stage C3 Practicalization Route and Stability",
            output,
            json_output,
...
def render_archpilot(campaign: dict[str, Any], output: Path, json_output: Path | None) -> None:
    _render_skip_report(
        "Escape Stage D Architecture-Adjacent Pilot",
        output,
        json_output,
        "The current oracle synthesis does not justify architecture-adjacent work, and this rescue/practicalization program did not alter that ceiling story.",
    )
...
def write_candidate_pack(campaign: dict[str, Any], output: Path) -> None:
    ...
    pack["provenance"]["deadlock_escape_notes"] = "detector-triggered rescue and practicalization program around the active round6 benchmark"
    output.parent.mkdir(parents=True, exist_ok=True)
...
def render_decision_memo(campaign: dict[str, Any], output: Path) -> None:
    ...
    elif gate_verdict == required and (bounded_rescue or (bool(route_ref.get("round6_pass")) and bool(stability_ref.get("round6_pass")) and stage1.get("selected_candidates"))):
        final_status = labels["confirm"]
    elif gate_verdict == required:
        final_status = labels["narrow"]
    else:
        final_status = labels["narrow_state"]
    lines = [
        "# Escape Program Decision Memo",
        "",
    ...
    if final_status == labels["replace"]:
        ...
        lines.append("- Rescue was real enough to justify a follow-on architecture branch, but the current benchmark lane did not produce a practicalized challenger and the public benchmark state does not change now.")
    elif final_status == labels["confirm"]:
        lines.append("- The benchmark stays on `round6`. The new program resolved the frontier around deadlock escape more sharply: bounded detector-triggered rescue has real but narrow value, while the accepted benchmark lane still lacks a replacement strong enough to move the public state.")
    elif final_status == labels["narrow"]:
        lines.append("- The benchmark stays on `round6`, but the rescue/practicalization program did not add enough durable headroom to support a stronger internal state than a narrower frontier around the current deadlock interpretation.")
    else:
        lines.append("- The rescue/practicalization program did not leave enough evidence to keep the current internal benchmark state as wide as it was. The state should narrow further.")
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="4-GPU deadlock escape, source-quality, and practicalization program around active round6")
