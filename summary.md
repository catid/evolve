...
- The DoorKey teacher-guided `SARE` result stays positive overall, and the new resume gate keeps it frozen:
  ...
  - the resume-gate audit finds a plausible but weak mechanism, not an actionable one: final-block seeds show high teacher confidence, higher disagreement, lower coverage, and some elevated route-pair concentration, but not one clean lever that earns a bounded retry
  ...
  - see [lss_resume_gate_failure_mechanism_report.md](outputs/reports/lss_resume_gate_failure_mechanism_report.md), [lss_frozen_claim_updated_combined_doorkey_report.md](outputs/reports/lss_frozen_claim_updated_combined_doorkey_report.md), and [lss_resume_gate_decision_memo.md](outputs/reports/lss_resume_gate_decision_memo.md)
...
- The current repo recommendation is to stay frozen as-is: keep the claim explicitly teacher-guided, DoorKey-only, and external-64-episode-only, and do not strengthen it into a PPO-only, specifically multi-expert, or cross-task routed advantage. See [lss_resume_gate_decision_memo.md](outputs/reports/lss_resume_gate_decision_memo.md).
